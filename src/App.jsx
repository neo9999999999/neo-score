...
function AIAnalysis({onSave}){
  ...
  async function generateFinal() {
    ...
    try {
      ...
      const userPrompt = (stockData && stockData.isBacktest ? "⚠️ 백테스트 모드 - 기준일: " + (stockData.baseDate||"") + ". 그 날짜 이후 데이터는 모르는 상태로 분석 종합해주세요.\n\n" : "") + "4개 분석 결과 (JSON):\n\n[NeoAi] " + JSON.stringify(aiResult).slice(0, 1500) +
  ...
  const analyze = async () => {
    ...
    if (stockData && stockData.isBacktest && stockData.futureDays && stockData.futureDays.length > 0) { try { const entryClose = stockData.todayPrice; const tp1Price = Math.round(entryClose * 1.10); const tp2Price = Math.round(entryClose * 1.20); const slPrice = Math.round(entryClose * 0.95); const fd = stockData.futureDays; let result = { entryClose, tp1Price, tp2Price, slPrice, hitTp1: null, hitTp2: null, hitSl: null, finalDay: null, finalClose: null, maxHigh: 0, minLow: Infinity, days: [] }; for (let i = 0; i < fd.length; i++) { const d = fd[i]; const dayInfo = { idx: i+1, date: d.date, open: d.open, high: d.high, low: d.low, close: d.close, hitTp1: d.high >= tp1Price, hitTp2: d.high >= tp2Price, hitSl: d.low <= slPrice }; result.days.push(dayInfo); if (d.high > result.maxHigh) result.maxHigh = d.high; if (d.low < result.minLow) result.minLow = d.low; if (!result.hitTp1 && d.high >= tp1Price) result.hitTp1 = i+1; if (!result.hitTp2 && d.high >= tp2Price) result.hitTp2 = i+1; if (!result.hitSl && d.low <= slPrice) result.hitSl = i+1; } result.finalDay = fd.length; result.finalClose = fd[fd.length-1].close; result.finalReturn = Math.round(((result.finalClose - entryClose) / entryClose) * 10000) / 100; if (result.hitSl && (!result.hitTp1 || result.hitSl <= result.hitTp1)) { result.outcome = "❌ 손절 (SL " + result.hitSl + "일차)"; result.outcomePct = -5; } else if (result.hitTp2) { result.outcome = "✅ TP2 도달 (" + result.hitTp2 + "일차, +20%)"; result.outcomePct = 20; } else if (result.hitTp1) { result.outcome = "✅ TP1 도달 (" + result.hitTp1 + "일차, +10%)"; result.outcomePct = 10; } else { result.outcome = "⏸️ 미도달 (" + result.finalReturn + "% 마감)"; result.outcomePct = result.finalReturn; } setVerifyResult(result); } catch(e) { console.error("[verify]", e); } }
    setLoading(false);
    setProgress("");
  };
      ...
      {hasResult && (
        <div>
          ...
          {activeTab === "ai" && (aiResult || chimResult || jdResult || hsResult) && (
            {verifyResult && (<div style={{marginTop:14,padding:16,background:verifyResult.outcomePct>=10?"linear-gradient(135deg,#d1fae5 0%,#a7f3d0 100%)":verifyResult.outcomePct<=-5?"linear-gradient(135deg,#fee2e2 0%,#fecaca 100%)":"linear-gradient(135deg,#e0e7ff 0%,#c7d2fe 100%)",border:"2px solid "+(verifyResult.outcomePct>=10?"#10b981":verifyResult.outcomePct<=-5?"#ef4444":"#6366f1"),borderRadius:12}}><div style={{fontSize:15,fontWeight:700,marginBottom:12}}>🔍 백테스트 검증 결과</div><div style={{fontSize:24,fontWeight:800,marginBottom:12}}>{verifyResult.outcome}</div><div style={{fontSize:13,marginBottom:8}}>진입가: <b>{verifyResult.entryClose.toLocaleString()}원</b> | TP1: <b>{verifyResult.tp1Price.toLocaleString()}</b> (+10%) | TP2: <b>{verifyResult.tp2Price.toLocaleString()}</b> (+20%) | SL: <b>{verifyResult.slPrice.toLocaleString()}</b> (-5%)</div><div style={{fontSize:13,marginBottom:8}}>관측기간 최고가: <b>{verifyResult.maxHigh.toLocaleString()}</b> | 최저가: <b>{verifyResult.minLow.toLocaleString()}</b> | {verifyResult.finalDay}일차 종가: <b>{verifyResult.finalClose.toLocaleString()}</b> ({verifyResult.finalReturn>=0?"+":""}{verifyResult.finalReturn}%)</div><details style={{marginTop:8,fontSize:12}}><summary style={{cursor:"pointer",fontWeight:600}}>일별 상세 (더보기)</summary><table style={{width:"100%",fontSize:11,marginTop:8,borderCollapse:"collapse"}}><thead><tr style={{background:"rgba(0,0,0,0.05)"}}><th style={{padding:4}}>일</th><th style={{padding:4}}>날짜</th><th style={{padding:4}}>시가</th><th style={{padding:4}}>고가</th><th style={{padding:4}}>저가</th><th style={{padding:4}}>종가</th><th style={{padding:4}}>TP1</th><th style={{padding:4}}>TP2</th><th style={{padding:4}}>SL</th></tr></thead><tbody>{verifyResult.days.map((d,i)=>(<tr key={i} style={{background:d.hitSl?"#fee2e2":d.hitTp2?"#a7f3d0":d.hitTp1?"#d1fae5":""}}><td style={{padding:4,textAlign:"center"}}>{d.idx}</td><td style={{padding:4,textAlign:"center"}}>{d.date}</td><td style={{padding:4,textAlign:"right"}}>{d.open.toLocaleString()}</td><td style={{padding:4,textAlign:"right"}}>{d.high.toLocaleString()}</td><td style={{padding:4,textAlign:"right"}}>{d.low.toLocaleString()}</td><td style={{padding:4,textAlign:"right"}}>{d.close.toLocaleString()}</td><td style={{padding:4,textAlign:"center"}}>{d.hitTp1?"✅":""}</td><td style={{padding:4,textAlign:"center"}}>{d.hitTp2?"✅":""}</td><td style={{padding:4,textAlign:"center"}}>{d.hitSl?"❌":""}</td></tr>))}</tbody></table></details></div>)}
    ...
    <div style={{background:"#fff",minHeight:"100vh",fontFamily:"-apple-system,'Pretendard',sans-serif",color:"#1e293b",fontSize:15,paddingBottom:68}}>
      ...
      <div style={{position:"fixed",bottom:0,left:0,right:0,background:"#fff",borderTop:"1px solid #e2e8f0",display:"flex",justifyContent:"center",zIndex:100}}>
        <div style={{display:"flex",maxWidth:1080,width:"100%",overflowX:"auto"}}>
          {[{id:"today",label:"네오오늘",icon:"🔥"},{id:"db",label:"네오스코어",icon:"🎯"},{id:"ai",label:"네오 Ai분석",icon:"🤖"},{id:"history",label:"히스토리",icon:"📋"}].map(t=>(
