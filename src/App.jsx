...
import { analyzeNeoAnalysis, NeoAnalysisResultCard, calcNeoAnalysisGrade, neoAnalysisGradeColor, calcNeoAnalysisFromRaw, NeoAnalysisDetailModal } from "./NeoAnalysisHelpers.jsx";
        ...
        <div onClick={() => setSel(null)} style={{position:"fixed", top:0, left:0, right:0, bottom:0, background:"rgba(0,0,0,0.6)", zIndex:1000, display:"flex", alignItems:"flex-start", justifyContent:"center", padding:"20px 12px", overflowY:"auto"}}>
          <div onClick={e => e.stopPropagation()} style={{background:"#fff", borderRadius:14, maxWidth:900, width:"100%", padding:0, position:"relative", marginBottom:40}}>
            <button onClick={() => setSel(null)} style={{position:"absolute", top:12, right:12, width:30, height:30, borderRadius:"50%", background:"#f1f5f9", border:"none", cursor:"pointer", fontSize:16, fontWeight:700, color:"#64748b", zIndex:2}}>✕</button>
              ...
              )}.scenarios && (<div style={{marginBottom:8}}>{detailTab === "ai" && (h[sel].detailedAnalysis || h[sel].keyReasons || h[sel].technicalIndicators || h[sel].supplyZone || h[sel].strategy) && (
                  <div style={{marginTop:12, padding:14, background:"#fefefe", border:"2px solid #c4b5fd", borderRadius:10, width:"100%", flexBasis:"100%", boxSizing:"border-box"}}>
                    <div style={{fontSize:14, fontWeight:700, color:"#7c3aed", marginBottom:10}}>🧠 네오분석 v1 상세</div>
              ...
              <div style={{display:"flex", borderBottom:"2px solid #e2e8f0", padding:"16px 16px 0", overflowX:"auto"}}>
                ...
                <div style={{marginTop:14, padding:14, background:"linear-gradient(135deg,#fef3c7,#fde68a)", border:"2px solid #f59e0b", borderRadius:10}}>
                  ...
                  {h[sel].finalResult><div style={{fontSize:11,fontWeight:600,color:"#78350f",marginBottom:4}}>📊 시나리오별 대응</div>{h[sel].finalResult.scenarios.bullish && <div style={{padding:6,background:"#dcfce7",borderRadius:4,fontSize:11,marginBottom:3}}><b>📈 강세:</b> {h[sel].finalResult.scenarios.bullish}</div>}{h[sel].finalResult.scenarios.neutral && <div style={{padding:6,background:"#f3f4f6",borderRadius:4,fontSize:11,marginBottom:3}}><b>➡️ 보합:</b> {h[sel].finalResult.scenarios.neutral}</div>}{h[sel].finalResult.scenarios.bearish && <div style={{padding:6,background:"#fee2e2",borderRadius:4,fontSize:11}}><b>📉 약세:</b> {h[sel].finalResult.scenarios.bearish}</div>}</div>)}
...
export default function App(){
  ...
  const showFromD=useCallback((r)=>setDetailModal({name:r.n,date:r.d,market:r.m,total:r.v1Total,grade:r.v1Grade,sections:{supply:{score:r.v1Supply,max:25},breakout:{score:r.v1Breakout,max:25},momentum:{score:r.v1Momentum,max:20},sectorMaterial:{score:r.v1Sm,max:15},accumulation:{score:r.v1Acc,max:15}}}),[]);
  const [detailModal,setDetailModal]=useState(null);
  const handleShowDetail=useCallback((s)=>{
    if(!s)return;
    const rawRow=[s.name,s.signal_date,s.market,+s.rate||0,(s.amount||s.vol||0)+"억",s.supply||s.investor||"",+s.score||0,s.grade];
  ...
  const saveAutoRule=useCallback((s)=>{
    ...
    try{judoRes=calcJudojuScore(s);}catch(e){}
    try{hsRes=calcHaseunghoonScore(s);}catch(e){}
    const rawRow=[s.name,s.signal_date,s.market,+s.rate||0,(s.amount||s.vol||0)+"억",s.supply||s.investor||"",+s.score||0,s.grade];
    try{neoRes=calcNeoAnalysisFromRaw(rawRow);}catch(e){}
    const entry={
      name:s.name,code:s.code,date:s.signal_date,market:s.market,
      grade:s.grade,score:s.score,breakType:"자동",
      investor:s.supply||s.investor||"",ema50:"-",rate:s.rate,
      sections:neoRes?neoRes.sections:null,
      total:neoRes?neoRes.total:(+s.score||0),
      summary:"자동 룰 매핑 결과 (raw 데이터 기반, 정확도 ~70%)",
      keyReasons:["등락률 "+s.rate+"%","거래대금 "+(s.amount||s.vol||0)+"억",s.supply||s.investor||"투자자 정보 없음"],
      risks:["raw 데이터 기반 자동 매핑 - 차트 패턴 미반영"],
      chimchakhaeResult:chimRes,judojuResult:judoRes,haseunghoonResult:hsRes,
      finalResult:{verdict:s.grade,summary:"4중 분석 자동 매핑 - 등급 "+s.grade,confidence:+s.score||50},
      tp1:s.tp1||10,tp2:s.tp2||20,sl:s.sl||-5,
      timestamp:Date.now()
    };
    saveHistory(entry);
  },[saveHistory]);
