...
import { loadUniverse, fetchYahooOHLCV, pMap, scoreAt, isCandidate, pickReason, fmtEok } from "./lib/stock-core.mjs";
...
async function main() {
  ...
  const candidates = universeByValue
    .filter(isCandidate)
    ...
    .map((x, idx) => ({
      ...
      reason: pickReason(x, x.name),
    }));

  console.log(`[afternoon] 분석 ${valid.length}종목 → 후보 ${candidates.length}종목`);

  const dir = v => v == null ? "혼조" : v > 0.1 ? "상승" : v < -0.1 ? "하락" : "보합";
  const summary = `미 선물은 나스닥 ${dir(nq)}(${nq >= 0 ? "+" : ""}${nq}%)·S&P ${dir(es)} 흐름으로, 익일 한국 증시는 ${marketBias === "bullish" ? "강세 우호" : marketBias === "bearish" ? "약세 경계" : "혼조"} 출발이 예상됩니다. 당일 거래대금 상위 종목 중 종가 강도·거래량·모멘텀이 강한 ${candidates.length}개를 익일 상승 후보로 선정했습니다.`;
  ...
  const report = {
    ...
    candidates,
    note: "미 선물·매크로 + 당일 종목 주가/거래량 기반 정량 스코어. 투자 참고용이며 책임은 본인에게 있습니다.",
  };
