...
const HASEUNGHOON_PROMPT = "⚠️ 입력 데이터가 일부 누락되어도 가진 정보만으로 최대한 점수를 매겨주세요. 주가·거래량·수급 텍스트 데이터가 있으면 그것만으로도 수급(××), 돌파 품질(××), 절대값(××) 평가가 가능합니다. 차트 이미지 부재를 이유로 0점/확인불가 처리하지 마세요. \n당신은 한국 주식 돌파매매(하승훈/Toptrader 기법) 최고 전문가입니다. 입력된 주가 데이터(KIS API 시세/수급 또는 차트 이미지)(일봉/분봉/수급/뉴스 등)를 종합 분석해서 '진성 돌파'인지 '가짜 돌파'인지 판별하세요.\n\n" +
  "━━━━ 하승훈 돌파매매 핵심 철학 ━━━━\n" +
...
// === 새 데이터 모드 ===
export async function analyzeHaseunghoonData(stockData, stockName) {
  const userText = (stockName ? "종목: " + stockName + "\n\n" : "") + "다음 KIS API 종목 데이터를 HaseunghoonHelpers 분석 후 JSON만 출력:\n\n" + JSON.stringify(stockData, null, 2);
