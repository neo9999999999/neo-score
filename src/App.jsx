  ...
  return(
    <div style={{background:"#fff",minHeight:"100vh",fontFamily:"-apple-system,'Pretendard',sans-serif",color:"#1e293b",fontSize:15,paddingBottom:68}}>
      ...
      <div style={{maxWidth:920,margin:"0 auto",padding:"20px 14px"}}>
        ...
        {(page==="db"||page==="ccdb"||page==="jddb"||page==="hsdb")&&<div style={{padding:"12px 12px 0",display:"flex",gap:6,flexWrap:"wrap"}}>{[{id:"db",l:"네오"},{id:"ccdb",l:"침착해"},{id:"jddb",l:"주도주"},{id:"hsdb",l:"하승훈"}].map(o=>(<button key={o.id} onClick={()=>{if(o.id==="jddb"||o.id==="hsdb"){alert(o.l+" 분석 DB는 준비중입니다. 침착해 또는 네오 분석을 사용하세요.");return;}setPage(o.id);}} style={{flex:"1 0 auto",padding:"8px 12px",borderRadius:8,border:"1px solid "+(page===o.id?"#1e293b":"#cbd5e1"),background:page===o.id?"#1e293b":"#fff",color:page===o.id?"#fff":"#475569",fontSize:12,fontWeight:page===o.id?700:500,cursor:"pointer"}}>{o.l}</button>))}</div>}{page==="db"&&<SignalDB/>}
        ...
        {page==="jddb"&&<div style={{padding:"40px 20px",textAlign:"center",color:"#64748b"}}>주도주 분석 DB는 준비중입니다.<br/>네오 또는 침착해 탭을 사용해주세요.</div>}
        {page==="hsdb"&&<div style={{padding:"40px 20px",textAlign:"center",color:"#64748b"}}>하승훈 분석 DB는 준비중입니다.<br/>네오 또는 침착해 탭을 사용해주세요.</div>}
