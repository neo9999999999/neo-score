...
const NEO_ANALYSIS_SYS_PROMPT = `⚠️ 입력 데이터가 일부 누락되어도 가진 정보만으로 최대한 점수를 매겨주세요. 주가·거래량·수급 텍스트 데이터가 있으면 그것만으로도 수급(××), 돌파 품질(××), 절대값(××) 평가가 가능합니다. 차트 이미지 부재를 이유로 0점/확인불가 처리하지 마세요. \n당신은 한국 주식 종가돌파매매 전문가입니다.
...
export async function analyzeNeoAnalysis(stockData, stockName) {
  let dataText = "";
  if (stockData && typeof stockData === "object" && !stockData.length) {
    ...
    content.push({ type: "text", text: "종목: " + (stockName || "미상") + "\n위 차트를 네오분석 v1 룰(5섹션 100점)로 채점하고 JSON만 출력." });
    return await callApi(content, NEO_ANALYSIS_SYS_PROMPT, 4500);
  } else {
    dataText = "종목: " + (stockName || "미상");
  }
  const userPrompt = dataText + "\n\n위 데이터로 네오분석 v1 룰(5섹션 100점) 채점. JSON만 출력.";
  return await callApi([{ type: "text", text: userPrompt }], NEO_ANALYSIS_SYS_PROMPT, 4500);
}

async function callApi(content, system, maxTokens) {
