...
function AIAnalysis({onSave}){
  ...
  const analyze = async () => {
    ...
    setLoading(true);
    setAiError(null); setChimError(null); setJdError(null);
    setAiResult(null); setChimResult(null); setJdResult(null);
    setProgress("AI분석 + 침착해 + 주도주 3중 분석 동시 실행 중...");
    ...
    const [aiRes, chimRes, jdRes] = await Promise.allSettled([aiPromise, chimPromise, jdPromise]);
  ...
  const hasResult = aiResult || chimResult || jdResult;
      ...
      {hasResult && (
        <div>
          ...
          <div style={{display:"flex", gap:0, marginBottom:14, borderBottom:"2px solid #e2e8f0"}}>
            <button onClick={() => setActiveTab("ai")} style={{flex:1, padding:"12px", border:"none", background:"transparent", borderBottom: activeTab==="ai" ? "3px solid #1e293b" : "3px solid transparent", marginBottom:"-2px", fontSize:13, fontWeight: activeTab==="ai" ? 800 : 600, color: activeTab==="ai" ? "#1e293b" : "#94a3b8", cursor:"pointer", fontFamily:"inherit"}}>
              🤖 AI {aiResult && <span style={{fontSize:11, color: gC(aiResult.grade), fontWeight:900, marginLeft:4}}>{aiResult.grade}</span>}
            </button>
            <button onClick={() => setActiveTab("chim")} style={{flex:1, padding:"12px", border:"none", background:"transparent", borderBottom: activeTab==="chim" ? "3px solid #7c3aed" : "3px solid transparent", marginBottom:"-2px", fontSize:13, fontWeight: activeTab==="chim" ? 800 : 600, color: activeTab==="chim" ? "#7c3aed" : "#94a3b8", cursor:"pointer", fontFamily:"inherit"}}>
              🎯 침착해 {chimResult && <span style={{fontSize:11, fontWeight:900, marginLeft:4}}>{chimResult.grade}</span>}
            </button>
            <button onClick={() => setActiveTab("jd")} style={{flex:1, padding:"12px", border:"none", background:"transparent", borderBottom: activeTab==="jd" ? "3px solid #ca8a04" : "3px solid transparent", marginBottom:"-2px", fontSize:13, fontWeight: activeTab==="jd" ? 800 : 600, color: activeTab==="jd" ? "#ca8a04" : "#94a3b8", cursor:"pointer", fontFamily:"inherit"}}>
    ...
    <div style={{background:"#fff",minHeight:"100vh",fontFamily:"-apple-system,'Pretendard',sans-serif",color:"#1e293b",fontSize:15,paddingBottom:68}}>
      ...
      <div style={{position:"fixed",bottom:0,left:0,right:0,background:"#fff",borderTop:"1px solid #e2e8f0",display:"flex",justifyContent:"center",zIndex:100}}>
        <div style={{display:"flex",maxWidth:920,width:"100%"}}>
          {[{id:"today",label:"오늘",icon:"🔥"},{id:"cctoday",label:"침오늘",icon:"🎯"},{id:"jdtoday",label:"주도주",icon:"📈"},{id:"db",label:"시그널DB",icon:"📊"},{id:"ccdb",label:"침DB",icon:"💎"},{id:"jddb",label:"주도주DB",icon:"🚀"},{id:"ai",label:"AI분석",icon:"🤖"},{id:"history",label:"히스토리",icon:"📋",badge:history.length}].map(t=>(
