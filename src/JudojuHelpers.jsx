...
const JUDOJU_PROMPT = "⚠️ 입력 데이터가 일부 누락되어도 가진 정보만으로 최대한 점수를 매겨주세요. 주가·거래량·수급 텍스트 데이터가 있으면 그것만으로도 수급(××), 돌파 품질(××), 절대값(××) 평가가 가능합니다. 차트 이미지 부재를 이유로 0점/확인불가 처리하지 마세요. \n당신은 한국 주식 주도주(主導株) 매매의 최고 전문가입니다. 입력된 주가 데이터(KIS API 시세/수급 또는 차트 이미지)(일봉/분봉/수급/섹터순위/뉴스 등)를 종합 분석해서 주도주로서의 가치를 판단하세요.\n\n" +
  "━━━━ 주도주 매매 핵심 철학 ━━━━\n" +
...
export async function analyzeJudoju(stockData, stockName) {
  let dataText = "";
  if (stockData && typeof stockData === "object" && !Array.isArray(stockData)) {
    ...
    content.push({ type: "text", text: "종목: " + (stockName || "미상") + "\n위 차트를 주도주 룰로 채점하고 JSON만 출력." });
    return await _callApi_analyzeJudoju(content, JUDOJU_PROMPT, 12000);
  } else {
    dataText = "종목: " + (stockName || "미상");
  }
  const userPrompt = dataText + "\n\n위 데이터로 주도주 룰로 채점하고 JSON만 출력.";
  return await _callApi_analyzeJudoju([{ type: "text", text: userPrompt }], JUDOJU_PROMPT, 12000);
}

async function _callApi_analyzeJudoju(content, system, maxTokens) {
...
// === 새 데이터 모드 ===
export async function analyzeJudojuData(stockData, stockName) {
  const userText = (stockName ? "종목: " + stockName + "\n\n" : "") + "다음 KIS API 종목 데이터를 JudojuHelpers 분석 후 JSON만 출력:\n\n" + JSON.stringify(stockData, null, 2);
