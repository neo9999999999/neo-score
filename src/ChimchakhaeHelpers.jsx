...
export async function analyzeChimchakhae(stockData, stockName) {
  let dataText = "";
  let isImage = false;
  ...
  if (data.content && Array.isArray(data.content)) {
    for (let i = 0; i < data.content.length; i++) { if (data.content[i].type === "text") text += data.content[i].text; }
  }
  ...
  if (!parsed) {
    ...
    if (fb !== -1 && lb !== -1) { try { parsed = JSON.parse(text.substring(fb, lb + 1)); } catch (e) {} }
  }
