...
<span style={{fontSize:13,color:"rgba(255,255,255,0.6)",fontWeight:500}}>원</span>
</div>
):(
<div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:6,marginBottom:10}}>
{["S","A","B"].map(gk=>(
<div key={gk} style={{display:"flex",alignItems:"center",gap:4}}>
<span style={{fontSize:13,fontWeight:700,color:gk==="S"?"#dc2626":gk==="A"?"#F5A623":"#5B8DEF",width:20}}>{gk}</span>
...
<div style={{fontSize:12,color:"rgba(255,255,255,0.55)",marginBottom:4,fontWeight:600,letterSpacing:"-0.2px"}}>{gk}급 ({d.n}건)</div>
<div style={{fontSize:14,fontWeight:700,color:c}}>{d.pnl>=0?"+":""}{d.pnl.toLocaleString()}원</div>
<div style={{fontSize:12,color:"rgba(255,255,255,0.5)",letterSpacing:"-0.2px",marginTop:2}}>투입 {d.amt.toLocaleString()} · {roi.toFixed(1)}% · 승 {d.n>0?(d.win/d.n*100).toFixed(0):0}%</div>
</div>);
})}
<div style={{padding:"10px 12px",background:"rgba(38,198,218,0.1)",borderRadius:10,borderLeft:"3px solid #26C6DA"}}>
<div style={{fontSize:12,color:"#26C6DA",marginBottom:4,fontWeight:700,letterSpacing:"-0.2px"}}>전체 ({portfolio.total.n}건)</div>
...
<div style={{fontSize:12,color:"rgba(255,255,255,0.5)",letterSpacing:"-0.2px",marginTop:2}}>투입 {portfolio.total.amt.toLocaleString()} · {portfolio.total.amt>0?(portfolio.total.pnl/portfolio.total.amt*100).toFixed(1):0}% · 승 {portfolio.total.n>0?(portfolio.total.win/portfolio.total.n*100).toFixed(0):0}%</div>
</div>
</div>
</div><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(148px,1fr))",gap:8,marginBottom:18}}>{["S","A","B"].map(g=>{const i=GI[g],s=st[g];return(<div key={g} onClick={()=>{setTab(g);setPg(0);setOpen(null)}} style={{background:tab===g?i.c:i.bg,borderRadius:14,padding:"12px 14px",cursor:"pointer",border:tab===g?"none":"0.5px solid "+i.bd}}><div style={{display:"flex",gap:6,margin:"8px 0",padding:"7px 10px",background:"#ffffff",borderRadius:8,border:"1px solid #e5e8eb",alignItems:"center",flexWrap:"wrap"}}><span style={{fontSize:13,fontWeight:700,color:"#475569"}}>전략설정</span><label style={{fontSize:12,color:"#64748b"}}>TP1<input type="number" value={cTP[g]?.tp1||""} onChange={(_g=>e=>setCTP(p=>({...p,[_g]:{...p[_g],tp1:+e.target.value}})))(g)} placeholder={NS[g]?.tp1} style={{width:40,margin:"0 4px",padding:"3px 5px",border:"1px solid #d1d6db",borderRadius:6,fontSize:13,fontWeight:600,color:"#191f28"}}/>%</label><label style={{fontSize:12,color:"#64748b"}}>TP2<input type="number" value={cTP[g]?.tp2||""} onChange={(_g=>e=>setCTP(p=>({...p,[_g]:{...p[_g],tp2:+e.target.value}})))(g)} placeholder={NS[g]?.tp2} style={{width:40,margin:"0 4px",padding:"3px 5px",border:"1px solid #d1d6db",borderRadius:6,fontSize:13,fontWeight:600,color:"#191f28"}}/>%</label><label style={{fontSize:12,color:"#64748b"}}>SL<input type="number" value={cTP[g]?.sl||""} onChange={(_g=>e=>setCTP(p=>({...p,[_g]:{...p[_g],sl:+e.target.value}})))(g)} placeholder={NS[g]?.sl} style={{width:40,margin:"0 4px",padding:"3px 5px",border:"1px solid #d1d6db",borderRadius:6,fontSize:13,fontWeight:600,color:"#191f28"}}/>%</label><label style={{fontSize:12,color:"#64748b"}}>강제SL<input type="number" value={cTP[g]?.fsl||""} onChange={(_g=>e=>setCTP(p=>({...p,[_g]:{...p[_g],fsl:+e.target.value}})))(g)} placeholder={NS[g]?.fsl||"off"} style={{width:40,margin:"0 4px",padding:"3px 5px",border:"1px solid #d1d6db",borderRadius:6,fontSize:13,fontWeight:600,color:"#191f28"}}/>%</label><button onClick={()=>{setCTP({...cTP,[g]:{tp1:0,tp2:0,sl:0,fsl:0}})}} style={{fontSize:12,padding:"3px 9px",background:"#191f28",color:"#fff",border:"none",borderRadius:6,cursor:"pointer",fontWeight:600,letterSpacing:"-0.2px"}}>초기화</button> <button onClick={()=>{const ds=D_live.filter(x=>x.g===g);let best=null;for(const t1 of [10,15,20,25,30])for(const t2 of [30,50,80,100])for(const sl of [3,5,7,10,15])for(const fsl of [0,7,10,15]){if(t2<=t1)continue;if(fsl>0&&fsl<sl)continue;const rs=ds.map(x=>simReal(x.ohlc,t1,t2,sl,fsl));const cum=rs.reduce((a,x)=>a+x.t,0);if(!best||cum>best.cum)best={t1,t2,sl,fsl,cum};}if(best)setCTP({...cTP,[g]:{tp1:best.t1,tp2:best.t2,sl:best.sl,fsl:best.fsl||0}});}} style={{fontSize:12,padding:"3px 9px",background:"#3182f6",color:"#fff",border:"none",borderRadius:6,cursor:"pointer",marginLeft:4,fontWeight:700,letterSpacing:"-0.2px"}}>수익MAX</button><button onClick={()=>{const ds=D_live.filter(x=>x.g===g);let best=null;for(const tp of [10,15,20,25,30,40,50,60,80,100])for(const sl of [3,5,7,10,15,20]){const rs=ds.map(x=>simReal(x.ohlc,tp,tp*2,sl,0));const cum=rs.reduce((a,x)=>a+x.t,0);if(!best||cum>best.cum)best={tp,sl,cum};}if(best)setCTP({...cTP,[g]:{tp1:best.tp,tp2:best.tp,sl:best.sl}});}} style={{fontSize:12,padding:"3px 9px",background:"#f04452",color:"#fff",border:"none",borderRadius:6,cursor:"pointer",marginLeft:4,fontWeight:700,letterSpacing:"-0.2px"}}>단일TP</button>{(()=>{const ds=D_live.filter(x=>x.g===g);const rs=ds.map(x=>x);const cum=Math.round(rs.reduce((a,x)=>a+x.t,0));const wr=ds.length?Math.round(rs.filter(x=>x.t>0).length/ds.length*100):0;const ev=ds.length?(rs.reduce((a,x)=>a+x.t,0)/ds.length).toFixed(2):0;return(<span style={{fontSize:13,fontWeight:700,marginLeft:8}}><span style={{color:"#2563eb"}}>누적{cum>0?"+":""}{cum}%</span> <span style={{color:"#dc2626"}}>승률{wr}%</span> <span style={{color:"#7c3aed"}}>EV={ev}</span></span>)})()}</div><div style={{display:"flex",justifyContent:"space-between"}}><span style={{fontSize:26,fontWeight:900,color:tab===g?"#fff":i.c}}>{g}</span></div><div style={{fontSize:22,fontWeight:900,color:tab===g?"#fff":i.c}}>{s.n}<span style={{fontSize:13,fontWeight:400}}>건</span></div><div style={{fontSize:13,color:tab===g?"rgba(255,255,255,0.8)":"#64748b",marginTop:2}}>+{s.avg}%/건 · 승률<strong>{s.wr}%</strong> · 현행+{s.cum}%</div><div style={{fontSize:12,color:tab===g?"rgba(255,255,255,0.65)":"#94a3b8",marginTop:1}}>TP1{s.tp1c}({s.tp1r}%) · TP2{s.boc}({s.bor}%) · SL{s.slc}({s.slr}%)</div></div>)})} <div style={{background:"#f1f5f9",borderRadius:14,padding:"12px 14px"}}><span style={{fontSize:26,fontWeight:900,color:"#94a3b8"}}>X</span><div style={{fontSize:22,fontWeight:900,color:"#94a3b8"}}>{XN}<span style={{fontSize:13,fontWeight:400}}>건</span></div><div style={{fontSize:13,color:"#94a3b8",marginTop:2}}>매수금지</div></div></div>{st[tab]&&<div style={{background:"#191f28",borderRadius:14,padding:"16px",marginBottom:12,color:"#fff"}}><div style={{display:"flex"}}><div style={{flex:1}}><div style={{fontSize:13,opacity:0.55,marginBottom:4,letterSpacing:"-0.2px",fontWeight:500}}>익절 (TP1+TP2)</div><div style={{fontSize:22,fontWeight:800,letterSpacing:"-0.5px",color:"#ff7a85"}}>{st[tab].tp1c}<span style={{fontSize:12,fontWeight:500,opacity:0.7,marginLeft:1}}>건</span></div><div style={{fontSize:13,opacity:0.6,marginTop:2,letterSpacing:"-0.2px"}}>{st[tab].tp1r}% · TP2 {st[tab].boc}건 ({st[tab].bor}%)</div></div><div style={{flex:1,borderLeft:"1px solid rgba(255,255,255,0.12)",paddingLeft:14}}><div style={{fontSize:13,opacity:0.55,marginBottom:4,letterSpacing:"-0.2px",fontWeight:500}}>손절 (SL)</div><div style={{fontSize:22,fontWeight:800,letterSpacing:"-0.5px",color:"#7eb6ff"}}>{st[tab].slc}<span style={{fontSize:12,fontWeight:500,opacity:0.7,marginLeft:1}}>건</span></div><div style={{fontSize:13,opacity:0.6,marginTop:2,letterSpacing:"-0.2px"}}>{Math.round((st[tab].slc)/st[tab].n*100)}%</div></div><div style={{flex:1,borderLeft:"1px solid rgba(255,255,255,0.12)",paddingLeft:14}}><div style={{fontSize:13,opacity:0.55,marginBottom:4,letterSpacing:"-0.2px",fontWeight:500}}>기간만료</div><div style={{fontSize:22,fontWeight:800,letterSpacing:"-0.5px",opacity:0.75}}>{st[tab].toc}<span style={{fontSize:12,fontWeight:500,opacity:0.7,marginLeft:1}}>건</span></div><div style={{fontSize:13,opacity:0.6,marginTop:2,letterSpacing:"-0.2px"}}>{st[tab].tor}%</div></div></div></div>}<div style={{borderRadius:12,border:"1px solid #e2e8f0",overflow:"hidden"}}><div style={{overflowX:"auto"}}><table style={{width:"100%",minWidth:760,borderCollapse:"collapse",fontSize:13}}><thead><tr style={{background:"#f8fafc"}}>{[{k:"d",l:"날짜"},{k:"n",l:"종목"},{k:"ch",l:"등락"},{k:"iv",l:"수급"},{k:"sc",l:"NEO"},{k:"r",l:""}].map(h=>(<th key={h.k} onClick={()=>ds(h.k)} style={{padding:"8px 5px",textAlign:h.k==="n"?"left":"center",fontWeight:600,fontSize:13,color:"#94a3b8",cursor:"pointer",borderBottom:"2px solid #e2e8f0",whiteSpace:"nowrap",userSelect:"none"}}>{h.l}{srt.c===h.k?(srt.d==="asc"?" ↑":" ↓"):""}</th>))}</tr></thead><tbody>{pd.map((r,i)=>{const isO=open===pg*PP+i,gi=GI[r.g];return[<tr key={"r"+i} onClick={()=>setOpen(isO?null:pg*PP+i)} style={{cursor:"pointer",borderBottom:"1px solid #f1f5f9",background:isO?gi.bg:"#fff"}}><td style={{padding:"8px 5px",textAlign:"center",fontSize:13,color:"#94a3b8"}}>{r.d.slice(2)}</td><td style={{padding:"8px 5px",fontWeight:700,fontSize:13,maxWidth:120,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{r.n}</td><td style={{padding:"8px 5px",textAlign:"center",fontWeight:700,color:"#dc2626",fontSize:12}}>+{r.ch}%</td><td style={{padding:"8px 5px",textAlign:"center",fontSize:13,fontWeight:600,color:r.iv==="기+외"?"#7c3aed":r.iv==="외인"?"#2563eb":"#94a3b8"}}>{r.iv}</td><td style={{padding:"8px 5px",textAlign:"center"}}><span style={{background:gi.c+"12",color:gi.c,padding:"1px 7px",borderRadius:20,fontWeight:800,fontSize:12}}>{r.sc}</span></td><td style={{padding:"8px 5px",textAlign:"center",fontWeight:800,fontSize:13,color:r.t>0?"#dc2626":"#2563eb"}}>{((r.t>0?"+":"")+r.t+"%"+(function(){var a=invAmt.useSame?invAmt.same:(invAmt[r.g]||0);if(!a)return"";var g=Math.round(a*r.t/100);return" ("+(g>=0?"+":"")+g.toLocaleString()+"원)";})())}</td><td style={{padding:"8px 5px",textAlign:"center"}}><span style={{padding:"1px 4px",borderRadius:4,fontSize:12,fontWeight:700,background:RC(r.r)+"12",color:RC(r.r)}}>{RL(r.r)}</span></td></tr>,isO&&(<tr key={"d"+i}><td colSpan={9} style={{padding:0}}><div style={{padding:"12px 16px",background:gi.bg,borderBottom:"2px solid "+gi.bd}}><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(80px,1fr))",gap:5,marginBottom:8}}>{[{l:"시총",v:r.mc},{l:"수급",v:r.iv},{l:"최대↑",v:"+"+r.pk+"%",c:"#dc2626"},{l:"최대↓",v:r.dd+"%",c:"#dc2626"},{l:"TP1도달일",v:r.tp1d?r.tp1d+(r.tp1dy?" ("+r.tp1dy+"일)":""):"-",c:r.tp1d?"#2563eb":"#94a3b8"},{l:"SL손절일",v:r.sld?r.sld+(r.sldy?" ("+r.sldy+"일)":""):"-",c:r.sld?"#dc2626":"#94a3b8"},{l:"TP2도달일",v:r.tp2d||"-",c:r.tp2d?"#dc2626":"#94a3b8"},{l:"청산일",v:r.exd?r.exd+(r.exdy?" ("+r.exdy+"일)":""):"-",c:"#64748b"}].map((x,j)=>(<div key={j} style={{background:"#fff",borderRadius:6,padding:"4px 6px"}}><div style={{fontSize:13,color:"#94a3b8"}}>{x.l}</div><div style={{fontSize:13,fontWeight:700,color:x.c||"#1e293b"}}>{x.v}</div></div>))}</div><div style={{background:"#fff",borderRadius:6,padding:"12px 14px"}}><div style={{display:"flex",alignItems:"baseline",gap:10,marginBottom:6,flexWrap:"wrap"}}><div style={{fontSize:22,fontWeight:900,color:r.t>=0?"#dc2626":"#2563eb"}}>실현 {r.t>0?"+":""}{r.t}%</div><div style={{fontSize:12,color:"#94a3b8",fontWeight:600}}>거래 시나리오</div></div><div style={{fontSize:13,color:"#475569",lineHeight:1.8}}>{SD(r,cTP)}</div></div></div></td></tr>)]})}</tbody></table></div></div><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:10}}><span style={{fontSize:12,color:"#94a3b8"}}>{fl.length}건 중 {pg*PP+1}~{Math.min((pg+1)*PP,fl.length)}</span><div style={{display:"flex",gap:4}}><button onClick={()=>setPg(Math.max(0,pg-1))} disabled={pg===0} style={{padding:"5px 12px",borderRadius:8,border:"1px solid #e2e8f0",background:"#fff",fontSize:13,fontWeight:700,cursor:pg===0?"default":"pointer",color:pg===0?"#e2e8f0":"#1e293b"}}>←</button><input type="number" value={pg+1} min={1} max={mx+1} onChange={e=>{const v=parseInt(e.target.value)-1;if(!isNaN(v)&&v>=0&&v<=mx)setPg(v);}} style={{padding:"5px 4px",fontSize:13,color:"#64748b",width:50,textAlign:"center",border:"1px solid #e2e8f0",borderRadius:6}}/><span style={{padding:"5px 2px",fontSize:13,color:"#94a3b8"}}>/{mx+1}</span><button onClick={()=>setPg(Math.min(mx,pg+1))} disabled={pg>=mx} style={{padding:"5px 12px",borderRadius:8,border:"1px solid #e2e8f0",background:"#fff",fontSize:13,fontWeight:700,cursor:pg>=mx?"default":"pointer",color:pg>=mx?"#e2e8f0":"#1e293b"}}>→</button></div></div></div>);}
...
</div><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(148px,1fr))",gap:8,marginBottom:18}}>{["S+","S","A+","A","B+","B","C"].map(g=>{const i=GI[g],s=st[g];return(<div key={g} onClick={()=>{setTab(g);setPg(0);setOpen(null)}} style={{background:tab===g?i.c:i.bg,borderRadius:14,padding:"12px 14px",cursor:"pointer",border:tab===g?"none":"0.5px solid "+i.bd}}><div style={{display:"flex",gap:6,margin:"8px 0",padding:"7px 10px",background:"#ffffff",borderRadius:8,border:"1px solid #e5e8eb",alignItems:"center",flexWrap:"wrap"}}><span style={{fontSize:13,fontWeight:700,color:"#475569"}}>전략설정</span><label style={{fontSize:12,color:"#64748b"}}>TP1<input type="number" value={cTP[g]?.tp1||""} onChange={(_g=>e=>setCTP(p=>({...p,[_g]:{...p[_g],tp1:+e.target.value}})))(g)} placeholder={NS[g]?.tp1} style={{width:40,margin:"0 4px",padding:"3px 5px",border:"1px solid #d1d6db",borderRadius:6,fontSize:13,fontWeight:600,color:"#191f28"}}/>%</label><label style={{fontSize:12,color:"#64748b"}}>TP2<input type="number" value={cTP[g]?.tp2||""} onChange={(_g=>e=>setCTP(p=>({...p,[_g]:{...p[_g],tp2:+e.target.value}})))(g)} placeholder={NS[g]?.tp2} style={{width:40,margin:"0 4px",padding:"3px 5px",border:"1px solid #d1d6db",borderRadius:6,fontSize:13,fontWeight:600,color:"#191f28"}}/>%</label><label style={{fontSize:12,color:"#64748b"}}>SL<input type="number" value={cTP[g]?.sl||""} onChange={(_g=>e=>setCTP(p=>({...p,[_g]:{...p[_g],sl:+e.target.value}})))(g)} placeholder={NS[g]?.sl} style={{width:40,margin:"0 4px",padding:"3px 5px",border:"1px solid #d1d6db",borderRadius:6,fontSize:13,fontWeight:600,color:"#191f28"}}/>%</label><label style={{fontSize:12,color:"#64748b"}}>강제SL<input type="number" value={cTP[g]?.fsl||""} onChange={(_g=>e=>setCTP(p=>({...p,[_g]:{...p[_g],fsl:+e.target.value}})))(g)} placeholder={NS[g]?.fsl||"off"} style={{width:40,margin:"0 4px",padding:"3px 5px",border:"1px solid #d1d6db",borderRadius:6,fontSize:13,fontWeight:600,color:"#191f28"}}/>%</label><button onClick={()=>{setCTP({...cTP,[g]:{tp1:0,tp2:0,sl:0,fsl:0}})}} style={{fontSize:12,padding:"3px 9px",background:"#191f28",color:"#fff",border:"none",borderRadius:6,cursor:"pointer",fontWeight:600,letterSpacing:"-0.2px"}}>초기화</button> <button onClick={()=>{const ds=D_live.filter(x=>x.ccG===g);let best=null;for(const t1 of [10,15,20,25,30])for(const t2 of [30,50,80,100])for(const sl of [3,5,7,10,15])for(const fsl of [0,7,10,15]){if(t2<=t1)continue;if(fsl>0&&fsl<sl)continue;const rs=ds.map(x=>simReal(x.ohlc,t1,t2,sl,fsl));const cum=rs.reduce((a,x)=>a+x.t,0);if(!best||cum>best.cum)best={t1,t2,sl,fsl,cum};}if(best)setCTP({...cTP,[g]:{tp1:best.t1,tp2:best.t2,sl:best.sl,fsl:best.fsl||0}});}} style={{fontSize:12,padding:"3px 9px",background:"#3182f6",color:"#fff",border:"none",borderRadius:6,cursor:"pointer",marginLeft:4,fontWeight:700,letterSpacing:"-0.2px"}}>수익MAX</button><button onClick={()=>{const ds=D_live.filter(x=>x.ccG===g);let best=null;for(const tp of [10,15,20,25,30,40,50,60,80,100])for(const sl of [3,5,7,10,15,20]){const rs=ds.map(x=>simReal(x.ohlc,tp,tp*2,sl,0));const cum=rs.reduce((a,x)=>a+x.t,0);if(!best||cum>best.cum)best={tp,sl,cum};}if(best)setCTP({...cTP,[g]:{tp1:best.tp,tp2:best.tp,sl:best.sl}});}} style={{fontSize:12,padding:"3px 9px",background:"#f04452",color:"#fff",border:"none",borderRadius:6,cursor:"pointer",marginLeft:4,fontWeight:700,letterSpacing:"-0.2px"}}>단일TP</button>{(()=>{const ds=D_live.filter(x=>x.ccG===g);const rs=ds.map(x=>x);const cum=Math.round(rs.reduce((a,x)=>a+x.t,0));const wr=ds.length?Math.round(rs.filter(x=>x.t>0).length/ds.length*100):0;const ev=ds.length?(rs.reduce((a,x)=>a+x.t,0)/ds.length).toFixed(2):0;return(<span style={{fontSize:13,fontWeight:700,marginLeft:8}}><span style={{color:"#2563eb"}}>누적{cum>0?"+":""}{cum}%</span> <span style={{color:"#dc2626"}}>승률{wr}%</span> <span style={{color:"#7c3aed"}}>EV={ev}</span></span>)})()}</div><div style={{display:"flex",justifyContent:"space-between"}}><span style={{fontSize:26,fontWeight:900,color:tab===g?"#fff":i.c}}>{g}</span></div><div style={{fontSize:22,fontWeight:900,color:tab===g?"#fff":i.c}}>{s.n}<span style={{fontSize:13,fontWeight:400}}>건</span></div><div style={{fontSize:13,color:tab===g?"rgba(255,255,255,0.8)":"#64748b",marginTop:2}}>+{s.avg}%/건 · 승률<strong>{s.wr}%</strong> · 현행+{s.cum}%</div><div style={{fontSize:12,color:tab===g?"rgba(255,255,255,0.65)":"#94a3b8",marginTop:1}}>TP1{s.tp1c}({s.tp1r}%) · TP2{s.boc}({s.bor}%) · SL{s.slc}({s.slr}%)</div></div>)})}</div>{st[tab]&&<div style={{background:"#191f28",borderRadius:14,padding:"16px",marginBottom:12,color:"#fff"}}><div style={{display:"flex"}}><div style={{flex:1}}><div style={{fontSize:13,opacity:0.55,marginBottom:4,letterSpacing:"-0.2px",fontWeight:500}}>익절 (TP1+TP2)</div><div style={{fontSize:22,fontWeight:800,letterSpacing:"-0.5px",color:"#ff7a85"}}>{st[tab].tp1c}<span style={{fontSize:12,fontWeight:500,opacity:0.7,marginLeft:1}}>건</span></div><div style={{fontSize:13,opacity:0.6,marginTop:2,letterSpacing:"-0.2px"}}>{st[tab].tp1r}% · TP2 {st[tab].boc}건 ({st[tab].bor}%)</div></div><div style={{flex:1,borderLeft:"1px solid rgba(255,255,255,0.12)",paddingLeft:14}}><div style={{fontSize:13,opacity:0.55,marginBottom:4,letterSpacing:"-0.2px",fontWeight:500}}>손절 (SL)</div><div style={{fontSize:22,fontWeight:800,letterSpacing:"-0.5px",color:"#7eb6ff"}}>{st[tab].slc}<span style={{fontSize:12,fontWeight:500,opacity:0.7,marginLeft:1}}>건</span></div><div style={{fontSize:13,opacity:0.6,marginTop:2,letterSpacing:"-0.2px"}}>{Math.round((st[tab].slc)/st[tab].n*100)}%</div></div><div style={{flex:1,borderLeft:"1px solid rgba(255,255,255,0.12)",paddingLeft:14}}><div style={{fontSize:13,opacity:0.55,marginBottom:4,letterSpacing:"-0.2px",fontWeight:500}}>기간만료</div><div style={{fontSize:22,fontWeight:800,letterSpacing:"-0.5px",opacity:0.75}}>{st[tab].toc}<span style={{fontSize:12,fontWeight:500,opacity:0.7,marginLeft:1}}>건</span></div><div style={{fontSize:13,opacity:0.6,marginTop:2,letterSpacing:"-0.2px"}}>{st[tab].tor}%</div></div></div></div>}<div style={{borderRadius:12,border:"1px solid #e2e8f0",overflow:"hidden"}}><div style={{overflowX:"auto"}}><table style={{width:"100%",minWidth:760,borderCollapse:"collapse",fontSize:13}}><thead><tr style={{background:"#f8fafc"}}>{[{k:"d",l:"날짜"},{k:"n",l:"종목"},{k:"ch",l:"등락"},{k:"iv",l:"수급"},{k:"sc",l:"NEO"},{k:"r",l:""}].map(h=>(<th key={h.k} onClick={()=>ds(h.k)} style={{padding:"8px 5px",textAlign:h.k==="n"?"left":"center",fontWeight:600,fontSize:13,color:"#94a3b8",cursor:"pointer",borderBottom:"2px solid #e2e8f0",whiteSpace:"nowrap",userSelect:"none"}}>{h.l}{srt.c===h.k?(srt.d==="asc"?" ↑":" ↓"):""}</th>))}</tr></thead><tbody>{pd.map((r,i)=>{const isO=open===pg*PP+i,gi=GI[r.ccG];return[<tr key={"r"+i} onClick={()=>{setOpen(isO?null:pg*PP+i);onRowClick&&onRowClick(r);}} style={{cursor:"pointer",borderBottom:"1px solid #f1f5f9",background:isO?gi.bg:"#fff"}}><td style={{padding:"8px 5px",textAlign:"center",fontSize:13,color:"#94a3b8"}}>{r.d.slice(2)}</td><td style={{padding:"8px 5px",fontWeight:700,fontSize:13,maxWidth:120,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{r.n}</td><td style={{padding:"8px 5px",textAlign:"center",fontWeight:700,color:"#dc2626",fontSize:12}}>+{r.ch}%</td><td style={{padding:"8px 5px",textAlign:"center",fontSize:13,fontWeight:600,color:r.iv==="기+외"?"#7c3aed":r.iv==="외인"?"#2563eb":"#94a3b8"}}>{r.iv}</td><td style={{padding:"8px 5px",textAlign:"center"}}><span style={{background:gi.c+"12",color:gi.c,padding:"1px 7px",borderRadius:20,fontWeight:800,fontSize:12}}>{r.sc}</span></td><td style={{padding:"8px 5px",textAlign:"center",fontWeight:800,fontSize:13,color:r.t>0?"#dc2626":"#2563eb"}}>{((r.t>0?"+":"")+r.t+"%"+(function(){var a=invAmt.useSame?invAmt.same:(invAmt[r.ccG]||0);if(!a)return"";var g=Math.round(a*r.t/100);return" ("+(g>=0?"+":"")+g.toLocaleString()+"원)";})())}</td><td style={{padding:"8px 5px",textAlign:"center"}}><span style={{padding:"1px 4px",borderRadius:4,fontSize:12,fontWeight:700,background:RC(r.r)+"12",color:RC(r.r)}}>{RL(r.r)}</span></td></tr>,isO&&(<tr key={"d"+i}><td colSpan={9} style={{padding:0}}><div style={{padding:"12px 16px",background:gi.bg,borderBottom:"2px solid "+gi.bd}}><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(80px,1fr))",gap:5,marginBottom:8}}>{[{l:"시총",v:r.mc},{l:"수급",v:r.iv},{l:"최대↑",v:"+"+r.pk+"%",c:"#dc2626"},{l:"최대↓",v:r.dd+"%",c:"#dc2626"},{l:"TP1도달일",v:r.tp1d?r.tp1d+(r.tp1dy?" ("+r.tp1dy+"일)":""):"-",c:r.tp1d?"#2563eb":"#94a3b8"},{l:"SL손절일",v:r.sld?r.sld+(r.sldy?" ("+r.sldy+"일)":""):"-",c:r.sld?"#dc2626":"#94a3b8"},{l:"TP2도달일",v:r.tp2d||"-",c:r.tp2d?"#dc2626":"#94a3b8"},{l:"청산일",v:r.exd?r.exd+(r.exdy?" ("+r.exdy+"일)":""):"-",c:"#64748b"}].map((x,j)=>(<div key={j} style={{background:"#fff",borderRadius:6,padding:"4px 6px"}}><div style={{fontSize:13,color:"#94a3b8"}}>{x.l}</div><div style={{fontSize:13,fontWeight:700,color:x.c||"#1e293b"}}>{x.v}</div></div>))}</div><div style={{background:"#fff",borderRadius:6,padding:"12px 14px"}}><div style={{display:"flex",alignItems:"baseline",gap:10,marginBottom:6,flexWrap:"wrap"}}><div style={{fontSize:22,fontWeight:900,color:r.t>=0?"#dc2626":"#2563eb"}}>실현 {r.t>0?"+":""}{r.t}%</div><div style={{fontSize:12,color:"#94a3b8",fontWeight:600}}>거래 시나리오</div></div><div style={{fontSize:13,color:"#475569",lineHeight:1.8}}>{SD(r,cTP)}</div></div></div></td></tr>)]})}</tbody></table></div></div><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:10}}><span style={{fontSize:12,color:"#94a3b8"}}>{fl.length}건 중 {pg*PP+1}~{Math.min((pg+1)*PP,fl.length)}</span><div style={{display:"flex",gap:4}}><button onClick={()=>setPg(Math.max(0,pg-1))} disabled={pg===0} style={{padding:"5px 12px",borderRadius:8,border:"1px solid #e2e8f0",background:"#fff",fontSize:13,fontWeight:700,cursor:pg===0?"default":"pointer",color:pg===0?"#e2e8f0":"#1e293b"}}>←</button><input type="number" value={pg+1} min={1} max={mx+1} onChange={e=>{const v=parseInt(e.target.value)-1;if(!isNaN(v)&&v>=0&&v<=mx)setPg(v);}} style={{padding:"5px 4px",fontSize:13,color:"#64748b",width:50,textAlign:"center",border:"1px solid #e2e8f0",borderRadius:6}}/><span style={{padding:"5px 2px",fontSize:13,color:"#94a3b8"}}>/{mx+1}</span><button onClick={()=>setPg(Math.min(mx,pg+1))} disabled={pg>=mx} style={{padding:"5px 12px",borderRadius:8,border:"1px solid #e2e8f0",background:"#fff",fontSize:13,fontWeight:700,cursor:pg>=mx?"default":"pointer",color:pg>=mx?"#e2e8f0":"#1e293b"}}>→</button></div></div></div>);}
...
const GradeRow=({label,col,arr,setArr,opts})=>(<div style={{marginBottom:5,display:'flex',alignItems:'center',gap:8}}><div style={{fontSize:13,fontWeight:700,color:col,letterSpacing:'-0.2px',width:48,flexShrink:0}}>{label}</div><div style={{flex:1,display:'flex',flexWrap:'wrap'}}>{opts.map(g=>(<Chip key={g} arr={arr} setArr={setArr} val={g}/>))}</div></div>);
...
<div style={{flex:1}}><div style={{fontSize:13,opacity:0.55,marginBottom:4,letterSpacing:'-0.2px',fontWeight:500}}>총 건수</div><div style={{fontSize:22,fontWeight:800,letterSpacing:'-0.5px'}}>{stats.n.toLocaleString()}<span style={{fontSize:12,fontWeight:500,opacity:0.7,marginLeft:1}}>건</span></div></div>
<div style={{flex:1,borderLeft:'1px solid rgba(255,255,255,0.12)',paddingLeft:12}}><div style={{fontSize:13,opacity:0.55,marginBottom:4,letterSpacing:'-0.2px',fontWeight:500}}>익절률</div><div style={{fontSize:22,fontWeight:800,letterSpacing:'-0.5px',color:'#ff7a85'}}>{(stats.p5/stats.n*100).toFixed(1)}<span style={{fontSize:12,fontWeight:500,opacity:0.8,marginLeft:1}}>%</span></div></div>
...
{/* 2행: 투자금 입력 */}
<div style={{fontSize:13,opacity:0.55,marginBottom:8,letterSpacing:'-0.2px',fontWeight:500}}>1건당 투자금</div>
...
{_amtPresets.map(a=>(<button key={a} onClick={()=>setInvAmt(a)} style={{padding:'5px 11px',borderRadius:16,border:'1px solid '+(invAmt===a?'#fff':'rgba(255,255,255,0.18)'),background:invAmt===a?'#fff':'transparent',color:invAmt===a?_T.text:'rgba(255,255,255,0.7)',fontSize:13,fontWeight:700,cursor:'pointer',letterSpacing:'-0.2px',transition:'all .12s'}}>{_man(a)}원</button>))}
</div>
{/* 3행: 환산 결과 */}
<div style={{display:'flex'}}>
<div style={{flex:1}}><div style={{fontSize:13,opacity:0.55,marginBottom:4,letterSpacing:'-0.2px',fontWeight:500}}>총 투입</div><div style={{fontSize:18,fontWeight:800,letterSpacing:'-0.4px'}}>{_man(stats.totalInvest)}<span style={{fontSize:13,fontWeight:500,opacity:0.7,marginLeft:1}}>원</span></div></div>
<div style={{flex:1,borderLeft:'1px solid rgba(255,255,255,0.12)',paddingLeft:12}}><div style={{fontSize:13,opacity:0.55,marginBottom:4,letterSpacing:'-0.2px',fontWeight:500}}>총 손익</div><div style={{fontSize:18,fontWeight:800,letterSpacing:'-0.4px',color:stats.totalPnl>=0?'#ff7a85':'#7eb6ff'}}>{stats.totalPnl>=0?'+':''}{_man(Math.abs(stats.totalPnl))}<span style={{fontSize:13,fontWeight:500,opacity:0.8,marginLeft:1}}>원</span></div></div>
<div style={{flex:1,borderLeft:'1px solid rgba(255,255,255,0.12)',paddingLeft:12}}><div style={{fontSize:13,opacity:0.55,marginBottom:4,letterSpacing:'-0.2px',fontWeight:500}}>수익률</div><div style={{fontSize:18,fontWeight:800,letterSpacing:'-0.4px',color:stats.ret>=0?'#ff7a85':'#7eb6ff'}}>{stats.ret>=0?'+':''}{stats.ret.toFixed(2)}<span style={{fontSize:13,fontWeight:500,opacity:0.8,marginLeft:1}}>%</span></div></div>
</div>
</div>)}
{/* 정렬 */}
...
<thead style={{position:'sticky',top:0,background:_T.linelt,zIndex:1}}><tr><th style={{padding:'10px 8px',textAlign:'left',fontSize:13,fontWeight:600,color:_T.hint,letterSpacing:'-0.2px'}}>종목</th><th style={{padding:'10px 6px',textAlign:'center',fontSize:13,fontWeight:600,color:_T.hint}}>날짜</th><th style={{padding:'10px 6px',textAlign:'center',fontSize:13,fontWeight:600,color:_T.hint}}>청산일</th><th style={{padding:'10px 6px',textAlign:'right',fontSize:13,fontWeight:600,color:_T.hint}}>등락</th><th style={{padding:'10px 4px',textAlign:'center',fontSize:13,fontWeight:600,color:_T.hint}}>네</th><th style={{padding:'10px 4px',textAlign:'center',fontSize:13,fontWeight:600,color:_T.hint}}>침</th><th style={{padding:'10px 4px',textAlign:'center',fontSize:13,fontWeight:600,color:_T.hint}}>주</th><th style={{padding:'10px 4px',textAlign:'center',fontSize:13,fontWeight:600,color:_T.hint}}>하</th><th style={{padding:'10px 6px',textAlign:'center',fontSize:13,fontWeight:600,color:_T.hint}}>결과</th><th style={{padding:'10px 8px',textAlign:'right',fontSize:13,fontWeight:600,color:_T.hint}}>수익</th></tr></thead>
<tbody>{filtered.slice(0,300).map((r,i)=>{const won=Math.round(invAmt*((r.t||0)/100));return(<tr key={i} onClick={()=>onRowClick&&onRowClick(r)} style={{cursor:'pointer',borderTop:'1px solid '+_T.linelt}}><td style={{padding:'10px 8px',fontWeight:600,color:_T.text,letterSpacing:'-0.2px'}}>{r.n}</td><td style={{padding:'10px 6px',textAlign:'center',color:_T.hint,fontSize:13}}>{r.d?r.d.slice(2):''}</td><td style={{padding:'10px 6px',textAlign:'center',color:_T.hint,fontSize:13}}>{r.exd||'—'}</td><td style={{padding:'10px 6px',textAlign:'right',color:(r.ch||0)>0?_T.up:_T.down,fontWeight:600}}>{r.ch}%</td><td style={{padding:'10px 4px',textAlign:'center',fontWeight:700,color:_T.body}}>{r.g}</td><td style={{padding:'10px 4px',textAlign:'center',fontWeight:700,color:_T.body}}>{r.ccG}</td><td style={{padding:'10px 4px',textAlign:'center',fontWeight:700,color:_T.body}}>{r.jdG}</td><td style={{padding:'10px 4px',textAlign:'center',fontWeight:700,color:_T.body}}>{r.hsG}</td><td style={{padding:'10px 6px',textAlign:'center',fontSize:13,fontWeight:600,color:_resCol(r.r)}}>{_resLbl(r.r)}</td><td style={{padding:'10px 8px',textAlign:'right',fontWeight:700}}><div style={{color:(r.t||0)>=0?_T.up:_T.down}}>{(r.t||0)>=0?'+':''}{(r.t||0).toFixed(1)}%</div><div style={{fontSize:12,color:_T.hint,fontWeight:500,marginTop:1}}>{won>=0?'+':''}{_man(Math.abs(won))}원</div></td></tr>);})}</tbody></table>
</div>
{filtered.length>300&&(<div style={{padding:'10px',textAlign:'center',color:_T.mute,fontSize:13,borderTop:'1px solid '+_T.line,background:_T.linelt}}>상위 300건만 표시 · 전체 {filtered.length.toLocaleString()}건</div>)}
</div>
</div>);
}
...
const _perfGradeMeta={
  S:{c:'#dc2626',l:'S',sub:'최상위',stat:'실현 +0.45% · 승률 35%'},
  A:{c:'#2563eb',l:'A',sub:'양호',stat:'실현 -1.25% · 승률 32%'},
  B:{c:'#8b95a1',l:'B',sub:'주의',stat:'실현 -2.67% · 승률 29%'}
};
// 청산 추천 — 다차원 백테스트 (시장 × 거래대금) 기반 (best01 universe n=754)
// 트레일+5% = 시초가+5%p 도달 즉익 / 미달 시 종가 매도 (분봉 검증 후 정확)
const _exitRec=(amt,mkt)=>{
  const isKS=mkt==='KOSPI'||mkt==='KS';
  const isKQ=mkt==='KOSDAQ'||mkt==='KO';
  // 1조+ KOSPI: 시초가 압도 (+1.91% vs 트레일 -0.35%, 차익실현 압력)
  if(amt>=10000&&isKS)return {l:'🐌 시초가',col:'#1f6dee',sub:'1조+ KOSPI: 시초가 +1.91% (트레일 -0.35%, 차익실현 압도)'};
  // KOSPI 2000~3000억: 시초가 베스트 (시초가 +0.41% vs 트레일 -0.99%)
  if(amt>=2000&&amt<3000&&isKS)return {l:'🐌 시초가',col:'#1f6dee',sub:'KOSPI 2~3천억: 시초가 +0.41% vs 종가 -2.24% (시초가 안전)'};
  // KOSPI 500~1000억: 종가 약간 우위
  if(amt<1000&&isKS)return {l:'📈 종가',col:'#0d8050',sub:'KOSPI 5~10백억: 종가 +1.36% (시초가 +0.83%)'};
  // KOSDAQ 1000~2000억: 시초가 베스트 (시초가 +1.34% vs 트레일 +1.32%)
  if(amt>=1000&&amt<2000&&isKQ)return {l:'🐌 시초가',col:'#1f6dee',sub:'KOSDAQ 1~2천억: 시초가 +1.34% (트레일과 비슷)'};
  // 그 외 모든 경우: 트레일+5% 베스트 (대부분의 케이스)
  // KOSDAQ 500~1k: T5 +2.02% / 5k~10k: T5 +2.47% / 3k~5k: T5 +1.47%
...
const [mode,setMode]=useState(()=>{try{
  // 강제 마이그레이션 — 한 번만 leader로 리셋 (v3에 best01 박혀있던 사용자 구제)
  if(!localStorage.getItem('nbdb_v3_migrated_2')){
    localStorage.setItem('nbdb_v3_migrated_2','1');
    localStorage.removeItem('nbdb_mode_v3');
    localStorage.removeItem('nbdb_mode_v2');
    return 'leader';
  }
  const v=localStorage.getItem('nbdb_mode_v3');
  const valid=['leader','neo25','neo90','best01','pullback','haseunghoon'];
...
{[
  ...
  {id:'pullback',l:'네오눌림목반등',sub:'-15% 추매 / TP+100%',col:'#0ea5e9'},
  {id:'haseunghoon',l:'하승훈 종가베팅',sub:'14:50~15:25 자동 평가',col:'#0d9488'}
...
{[{id:'open',l:'🐌 D+1 시초가',col:'#1f6dee'},{id:'close',l:'📈 D+1 종가',col:'#0d8050'},{id:'trail',l:'🚀 트레일링 (5%↑/-3%)',col:'#10b981'}].map(e=>{const a=exitMethod===e.id;return(
  <button key={e.id} onClick={()=>setExitMethod(e.id)} style={{flex:'1 1 0',padding:'8px 6px',border:'none',background:a?e.col:'transparent',color:a?'#fff':_T.sub,borderRadius:8,cursor:'pointer',fontSize:13,fontWeight:a?800:600,letterSpacing:'-0.2px',whiteSpace:'nowrap',minWidth:0,overflow:'hidden',textOverflow:'ellipsis'}}>{e.l}</button>);})}
</div>
{/* 필터 카드 */}
...
{(<div style={{fontSize:13,color:_T.sub,fontWeight:500,lineHeight:1.6,padding:'8px 10px',background:_T.linelt,borderRadius:8,marginBottom:10,letterSpacing:'-0.2px'}}>
{mode==='leader'&&(<>
  · 등락률 +10% ~ +28%<br/>
...
{['S','A','B'].map(g=>{const on=selGrade.includes(g);const meta=_perfGradeMeta[g];return(<button key={g} title={'추천등급 '+g+' · '+meta.stat} onClick={()=>setSelGrade(p=>p.includes(g)?p.filter(x=>x!==g):[...p,g])} style={{padding:'4px 11px',borderRadius:6,border:'1px solid '+(on?meta.c:_T.line),background:on?meta.c:_T.bg,color:on?'#fff':_T.body,fontSize:12,fontWeight:on?800:600,cursor:'pointer'}}><b>{g}</b><span style={{fontSize:11,fontWeight:500,opacity:0.85,marginLeft:4}}>{meta.sub}</span></button>);})}
</div>
{/* 수급 필터 — 멀티선택 On/off */}
<div style={{display:'flex',gap:5,marginTop:8,flexWrap:'wrap',alignItems:'center'}}>
...
{exitStats&&(()=>{
  ...
  return(<div style={{background:_T.card,borderRadius:14,padding:'14px 16px',marginBottom:10,color:_T.text,border:'1px solid '+_T.line}}>
    <div style={{fontSize:12,fontWeight:700,color:_T.text,marginBottom:8,letterSpacing:'-0.3px',display:'flex',alignItems:'baseline',gap:8}}>
      📊 청산룰 평균 비교 <span style={{fontSize:12,color:'#10b981',fontWeight:700}}>(클릭하여 적용)</span>
      <span style={{fontSize:12,color:_T.hint,fontWeight:500}}>{exitStats.n}건 기준</span>
    </div>
    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8}}>
      {items.map(x=>{
        ...
        return(<div key={x.k} onClick={()=>setExitMethod(x.k)} style={{padding:'10px 8px',borderRadius:9,background:isSelected?x.col+'18':_T.bg,border:'2px solid '+(isSelected?x.col:_T.line),textAlign:'center',cursor:'pointer',transition:'all 0.15s'}}>
          <div style={{fontSize:12,fontWeight:700,color:x.col,letterSpacing:'-0.2px',marginBottom:4}}>{x.l}{isBest&&' ★'}{isSelected&&' ✓'}</div>
          <div style={{fontSize:18,fontWeight:800,color:d.avg>=0?_T.up:_T.down,letterSpacing:'-0.3px'}}>{d.avg>=0?'+':''}{d.avg.toFixed(2)}%</div>
          <div style={{fontSize:13,color:_T.sub,fontWeight:600,marginTop:2}}>승률 {d.winRate.toFixed(0)}%</div>
        </div>);
      })}
    </div>
  </div>);
...
<div key={x.rank} style={{padding:'10px',borderRadius:10,background:_T.bg,border:'1px solid '+_T.line,textAlign:'center'}}>
  <div style={{display:'inline-block',padding:'2px 8px',borderRadius:5,background:x.col,color:'#fff',fontSize:12,fontWeight:900,marginBottom:6}}>{x.rank}등</div>
  <div style={{fontSize:18,fontWeight:800,color:x.d.avg>=0?_T.up:_T.down,letterSpacing:'-0.3px'}}>{x.d.avg>=0?'+':''}{x.d.avg.toFixed(2)}%</div>
  <div style={{fontSize:12,color:_T.sub,marginTop:3}}>n={x.d.n} · 익절 {x.d.winRate.toFixed(0)}%</div>
</div>))}
</div>
<div style={{display:'flex',padding:'10px 12px',background:_T.linelt,borderRadius:8,gap:14,alignItems:'baseline'}}>
<div style={{fontSize:13,color:_T.hint,fontWeight:600}}>이벤트 가중평균</div>
...
<div style={{fontSize:12,color:_T.sub,marginLeft:'auto'}}>{leaderStats.events}이벤트 · 0.5×1등 + 0.335×2등 + 0.165×3등</div>
</div>
</div>)}
...
{[
  ...
  return(<div key={x.k} style={{padding:'10px',borderRadius:10,background:_T.bg,border:'1px solid '+x.col,textAlign:'center'}}>
    <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:5}}>
      <span style={{fontSize:13,fontWeight:800,color:x.col,letterSpacing:'-0.2px'}}>{x.l}</span>
      {x.allPos&&<span style={{fontSize:13,color:'#10b981',fontWeight:800}}>✅</span>}
    </div>
    <div style={{fontSize:13,color:_T.sub,fontWeight:600,marginTop:1}}>{x.sub}</div>
    ...
    <div style={{fontSize:13,color:_T.hint,fontWeight:600,marginTop:1}}>비용차감 (원평균 +{d.avg.toFixed(2)}%)</div>
    <div style={{fontSize:12,color:_T.body,fontWeight:600,marginTop:4}}>익절 {d.winRate.toFixed(0)}% · 손익비 {d.ratio.toFixed(2)}</div>
    <div style={{fontSize:13,color:d.cumAdj>=0?_T.up:_T.down,fontWeight:700,marginTop:2}}>5년누적 {d.cumAdj>=0?'+':''}{d.cumAdj.toFixed(0)}%</div>
  </div>);
})}
</div>
{/* 연도별 × 청산 비교 표 */}
<div style={{padding:'10px 12px',background:_T.linelt,borderRadius:8,marginBottom:10}}>
<div style={{fontSize:12,fontWeight:700,color:_T.hint,marginBottom:6,letterSpacing:'-0.2px'}}>연도별 × 청산 (비용 차감 후 평균)</div>
<div style={{fontSize:13,fontWeight:600}}>
<div style={{display:'grid',gridTemplateColumns:'42px repeat(6,1fr) 50px',gap:4,fontSize:12,fontWeight:700,color:_T.hint,marginBottom:4,padding:'0 2px'}}>
...
{[
  ...
  <div key={row.k} style={{display:'grid',gridTemplateColumns:'42px repeat(6,1fr) 50px',gap:4,padding:'5px 2px',borderTop:'1px solid '+_T.line,alignItems:'center'}}>
    <span style={{fontSize:13,fontWeight:800,color:row.col,letterSpacing:'-0.2px'}}>{row.l}</span>
    {best01Stats.yrAvgs.map(yr=>{
      const d=yr[row.k];if(!d)return <span key={yr.y}>—</span>;
      const v=d.adjAvg;
      return(<span key={yr.y} style={{textAlign:'center',fontSize:13,fontWeight:700,color:v>=0?_T.up:_T.down,letterSpacing:'-0.2px'}}>{v>=0?'+':''}{v.toFixed(2)}%</span>);
    })}
    <span style={{textAlign:'right',fontSize:12,color:_T.sub,fontWeight:600}}>{best01Stats[row.k]?.n||0}</span>
  </div>
))}
</div>
</div>
...
<div style={{fontSize:12,fontWeight:700,color:_T.hint,marginBottom:5,letterSpacing:'-0.2px'}}>Tier 분포 (어느 룰에 속하는지)</div>
<div style={{display:'flex',gap:8,flexWrap:'wrap',fontSize:13,fontWeight:600,color:_T.body}}>
...
<div style={{fontSize:13,fontWeight:800,color:'#a855f7',marginBottom:4,letterSpacing:'-0.2px'}}>📡 트레일링 검증 데이터 누적 중</div>
<div style={{fontSize:12,color:_T.body,fontWeight:500,lineHeight:1.6,letterSpacing:'-0.2px'}}>
  · KIS 1분봉 자동 수집 (장 마감 5분 후 cron) → <code style={{background:_T.bg,padding:'1px 5px',borderRadius:3,fontSize:13}}>data/intraday/&#123;date&#125;.json</code><br/>
...
<div><div style={{fontSize:13,color:_T.hint,marginBottom:6,letterSpacing:'-0.2px',fontWeight:600}}>총 건수 {yf.length>0?'('+yf.join('+')+'년)':'(전체)'}</div><div style={{fontSize:24,fontWeight:800,letterSpacing:'-0.5px',color:_T.text}}>{stats.n.toLocaleString()}<span style={{fontSize:12,fontWeight:500,color:_T.sub,marginLeft:2}}>건</span></div></div>
<div style={{borderLeft:'1px solid '+_T.line,paddingLeft:14}}><div style={{fontSize:13,color:_T.hint,marginBottom:6,letterSpacing:'-0.2px',fontWeight:600}}>익절률</div><div style={{fontSize:24,fontWeight:800,letterSpacing:'-0.5px',color:_T.up}}>{stats.winRate.toFixed(1)}<span style={{fontSize:12,fontWeight:500,opacity:0.8,marginLeft:2}}>%</span></div></div>
...
<div><div style={{fontSize:13,color:_T.hint,marginBottom:6,letterSpacing:'-0.2px',fontWeight:600}}>✅ 적중</div><div style={{fontSize:22,fontWeight:800,letterSpacing:'-0.5px',color:_T.up}}>{stats.totalHit.toLocaleString()}<span style={{fontSize:12,fontWeight:500,color:_T.sub,marginLeft:2}}>건</span></div></div>
...
{[
  ...
  return(<div key={x.k} style={{padding:'12px 14px',borderRadius:10,background:_T.bg,border:'1px solid '+(x.n>0?x.col:_T.line)}}>
    <div style={{display:'flex',alignItems:'baseline',justifyContent:'space-between',marginBottom:6}}>
      ...
      <span style={{fontSize:12,color:_T.hint,fontWeight:600}}>{x.sub}</span>
    </div>
    <div style={{display:'flex',alignItems:'baseline',gap:6}}>
      <span style={{fontSize:22,fontWeight:800,color:_T.text,letterSpacing:'-0.3px'}}>{x.n}</span>
      <span style={{fontSize:13,color:_T.sub,fontWeight:600}}>건 ({pct.toFixed(1)}%)</span>
    </div>
    {x.n>0&&<div style={{fontSize:13,color:x.avg>=0?_T.up:_T.down,fontWeight:700,marginTop:3,letterSpacing:'-0.2px'}}>평균 {x.avg>=0?'+':''}{x.avg.toFixed(2)}%</div>}
  </div>);
})}
</div>
{(stats.cnt.flat>0||stats.cnt.live>0)&&(<div style={{padding:'8px 12px',background:_T.linelt,borderRadius:8,marginBottom:12,fontSize:13,color:_T.body,fontWeight:600,letterSpacing:'-0.2px',display:'flex',gap:14,flexWrap:'wrap',alignItems:'center'}}>
...
{stats.totalCheck>0&&(<div style={{padding:'10px 12px',background:_T.linelt,borderRadius:8,marginBottom:12}}>
  <div style={{fontSize:12,fontWeight:700,color:_T.hint,marginBottom:5,letterSpacing:'-0.2px'}}>추천별 적중 (어느 추천이 더 잘 맞나?)</div>
  ...
  {[
    ...
    if(!d||d.n===0)return(<div key={x.k} style={{padding:'6px',borderRadius:6,background:_T.bg,border:'1px solid '+_T.line,textAlign:'center'}}>
      <div style={{fontSize:12,fontWeight:700,color:_T.mute}}>{x.l}</div>
      <div style={{fontSize:13,color:_T.mute,marginTop:1}}>0건</div>
    </div>);
    ...
    return(<div key={x.k} style={{padding:'6px',borderRadius:6,background:_T.bg,border:'1px solid '+x.col,textAlign:'center'}}>
      <div style={{fontSize:12,fontWeight:800,color:x.col,letterSpacing:'-0.2px'}}>{x.l}</div>
      <div style={{fontSize:13,fontWeight:800,color:_T.text,marginTop:1,letterSpacing:'-0.3px'}}>{acc.toFixed(0)}<span style={{fontSize:13,fontWeight:600,color:_T.sub,marginLeft:1}}>%</span> <span style={{fontSize:13,color:_T.sub,fontWeight:600}}>({d.hit}/{d.n})</span></div>
    </div>);
  })}
  </div>
</div>)}
<div style={{height:1,background:_T.line,margin:'0 -18px 16px'}}/>
<div style={{fontSize:13,color:_T.hint,marginBottom:8,letterSpacing:'-0.2px',fontWeight:600}}>1건당 투자금</div>
<div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}>
...
{_amtPresets.map(a=>(<button key={a} onClick={()=>setInvAmt(a)} style={{padding:'6px 12px',borderRadius:18,border:'1px solid '+(invAmt===a?_T.accent:_T.line),background:invAmt===a?_T.accent:'transparent',color:invAmt===a?'#fff':_T.body,fontSize:13,fontWeight:700,cursor:'pointer',letterSpacing:'-0.2px',transition:'all .15s'}}>{_man(a)}원</button>))}
</div>
<div style={{display:'flex'}}>
<div style={{flex:1}}><div style={{fontSize:13,color:_T.hint,marginBottom:6,letterSpacing:'-0.2px',fontWeight:600}}>총 투입</div><div style={{fontSize:18,fontWeight:800,letterSpacing:'-0.4px',color:_T.text}}>{_man(stats.totalInvest)}<span style={{fontSize:13,fontWeight:500,color:_T.sub,marginLeft:2}}>원</span></div></div>
<div style={{flex:1,borderLeft:'1px solid '+_T.line,paddingLeft:14}}><div style={{fontSize:13,color:_T.hint,marginBottom:6,letterSpacing:'-0.2px',fontWeight:600}}>총 손익</div><div style={{fontSize:18,fontWeight:800,letterSpacing:'-0.4px',color:stats.totalPnl>=0?_T.up:_T.down}}>{stats.totalPnl>=0?'+':''}{_man(Math.abs(stats.totalPnl))}<span style={{fontSize:13,fontWeight:500,opacity:0.8,marginLeft:2}}>원</span></div></div>
...
{r._tierLabel&&<span style={{fontSize:12,fontWeight:800,color:'#fff',background:r._tierCol,padding:'3px 8px',borderRadius:4,letterSpacing:'-0.2px'}}>{r._tierLabel}</span>}
{(()=>{
  const rec=_exitRec(_amtNum(r.mc),r.m);
...
{(()=>{const pg=_perfGrade(r);const m=_perfGradeMeta[pg];return(<span title={'OOS 추천등급 '+pg+' ('+m.sub+') · '+m.stat} style={{fontSize:13,fontWeight:900,color:'#fff',background:m.c,padding:'3px 9px',borderRadius:5,letterSpacing:'-0.2px',flexShrink:0}}>{pg}</span>);})()}
    ...
    <div style={{padding:"16px 18px",borderRadius:13,border:"1px solid "+_T.line,marginBottom:10,background:_T.card,transition:"all .15s"}} onMouseEnter={e=>{e.currentTarget.style.borderColor=accent+"60";}} onMouseLeave={e=>{e.currentTarget.style.borderColor=_T.line;}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10,marginBottom:12}}>
        <div style={{flex:1,minWidth:0,display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
          ...
          <button onClick={_copy} style={{fontSize:13,fontWeight:700,padding:"4px 10px",borderRadius:6,border:"1px solid "+_T.line,background:_T.bg,color:_T.body,cursor:"pointer",letterSpacing:"-0.2px",transition:"all .15s"}}>복사</button>
          <span title={exitR.sub} style={{fontSize:13,fontWeight:800,color:"#fff",background:exitR.col,padding:"3px 9px",borderRadius:5,letterSpacing:"-0.2px"}}>{exitR.l}</span>
        </div>
      ...
      <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",padding:"12px 14px",background:_T.bg,borderRadius:10,border:"1px solid "+_T.line}}>
        ...
        <span style={{color:_T.mute,fontSize:13}}>·</span>
        {/* 점수 */}
        <span style={{fontSize:12,fontWeight:700,color:_T.body}}>점수 <b style={{color:_T.text}}>{s.score}</b></span>
        {/* 신고가 */}
        {s.h120===1?(<span style={{fontSize:13,fontWeight:700,color:_T.green,background:"rgba(16,185,129,0.12)",border:"1px solid rgba(16,185,129,0.35)",padding:"3px 9px",borderRadius:5,letterSpacing:"-0.2px"}}>120일↑</span>):s.h60===1&&(<span style={{fontSize:13,fontWeight:700,color:_T.green,background:"rgba(16,185,129,0.08)",border:"1px solid rgba(16,185,129,0.25)",padding:"3px 9px",borderRadius:5,letterSpacing:"-0.2px"}}>60일↑</span>)}
        {/* 프로그램 */}
        {prog!==0&&(<span style={{fontSize:13,fontWeight:700,color:progPos?_T.green:_T.down,background:progPos?"rgba(16,185,129,0.12)":"rgba(88,166,255,0.12)",border:"1px solid "+(progPos?"rgba(16,185,129,0.35)":"rgba(88,166,255,0.35)"),padding:"3px 9px",borderRadius:5}}>프로그램 {_fmtKQty(prog)}</span>)}
        {/* 우측: 주가 + 시장 */}
...
if(!data)return(<div style={{padding:"12px",background:_T.bg,minHeight:"100vh"}}>
<div style={{display:"flex",gap:6,marginBottom:12,flexWrap:"wrap"}}>{[{id:"best01",l:"최고조합01",col:"#f59e0b"},{id:"leader",l:"네오 대장주",col:"#a855f7"},{id:"neo25",l:"네오 25%",col:"#c81e1e"},{id:"pullback",l:"네오눌림목반등",col:"#0ea5e9"},{id:"haseunghoon",l:"하승훈 종가베팅",col:"#0d9488"},{id:"oos",l:"📚 OOS 히스토리",col:"#7c3aed"}].map(t=>{const a=activeTab===t.id;return(<button key={t.id} onClick={()=>setActiveTab(t.id)} style={{flex:"1 1 0",padding:"12px 10px",borderRadius:11,border:"1px solid "+(a?t.col:_T.line),background:a?t.col:_T.card,color:a?"#fff":_T.body,cursor:"pointer",fontSize:13,fontWeight:800,letterSpacing:"-0.3px",whiteSpace:"nowrap"}}>{t.l}</button>);})}</div>
<div style={{padding:"60px 24px",textAlign:"center",background:_T.card,borderRadius:14,border:"1px solid "+_T.line}}><div style={{fontSize:42,marginBottom:14}}>📡</div><div style={{fontSize:17,fontWeight:800,color:_T.text,marginBottom:10,letterSpacing:"-0.3px"}}>네오 종배 신호 조회</div><div style={{fontSize:12,color:_T.sub,marginBottom:24,lineHeight:1.7}}>등락 15-29% / 거래대금 100억+ / 기관·기+외 / 점수3+ / 120일 신고가<br/>네오 7% (소형~중형) / 네오 25% (대장주) 자동 분류</div>{err&&<div style={{padding:"10px 14px",borderRadius:9,background:"rgba(248,81,73,0.12)",border:"1px solid rgba(248,81,73,0.35)",color:_T.up,fontSize:12,marginBottom:14}}>⚠️ {err}</div>}<button onClick={load} style={{padding:"14px 32px",borderRadius:11,border:"none",background:_T.accent,color:"#fff",fontSize:14,fontWeight:800,cursor:"pointer",letterSpacing:"-0.3px"}}>📡 KIS 조회</button></div></div>);
// 메인 화면 (다크 + 좌우 탭)
const _tabConf={
...
<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10,padding:"12px 14px",background:_T.card,border:"1px solid "+_T.line,borderRadius:12}}>
  <div>
    <div style={{fontSize:13,fontWeight:700,color:_T.text,letterSpacing:"-0.3px"}}>{data.date} · {data.time}{(()=>{const dt=new Date();const day=dt.getDay();return (day===0||day===6)?<span style={{fontSize:12,fontWeight:600,color:"#f59e0b",marginLeft:6,padding:"1px 6px",borderRadius:4,background:"rgba(245,158,11,0.15)"}}>최근 영업일 종가</span>:null;})()}</div>
    <div style={{fontSize:13,color:_T.hint,marginTop:3}}>총 {(data.summary&&data.summary.total)||0}건 · 25% <b style={{color:"#c81e1e"}}>{_tabConf.neo25.cnt}</b> · 대장주 <b style={{color:"#a855f7"}}>{_tabConf.leader.cnt}</b> · 최고01 <b style={{color:"#f59e0b"}}>{_tabConf.best01.cnt}</b> · 미통과 {(data.summary&&data.summary.cetc)||0}</div>
  </div>
  ...
  {["best01","leader","neo25","pullback","haseunghoon","oos"].map(tk=>{
    ...
    return(
      <button key={tk} onClick={()=>setActiveTab(tk)} style={{flex:"1 1 0",padding:"14px 12px",borderRadius:11,border:"1px solid "+(a?t.col:_T.line),background:a?t.col:_T.card,color:a?"#fff":_T.body,cursor:"pointer",transition:"all .12s",textAlign:"left"}}>
        ...
        <div style={{fontSize:12,fontWeight:600,opacity:a?0.85:0.7,letterSpacing:"-0.2px"}}>익절 {t.winRate} · 평균 {t.avg}</div>
      </button>);
  })}
</div>
...
<div style={{padding:"12px 14px",background:_T.card,border:"1px solid "+_T.line,borderRadius:12,marginBottom:10}}>
  <div style={{display:"flex",gap:8,alignItems:"baseline",marginBottom:8}}>
    <span style={{fontSize:13,fontWeight:700,color:tab.col,letterSpacing:"-0.2px"}}>📋 진입 조건</span>
    <span style={{fontSize:12,color:_T.hint,fontWeight:500}}>{tab.strategy}</span>
  </div>
  {activeTab==='leader'?(<div style={{fontSize:13,color:_T.body,fontWeight:500,lineHeight:1.7,letterSpacing:"-0.2px",marginBottom:10}}>
  ...
  </div>):activeTab==='best01'?(<div style={{fontSize:13,color:_T.body,fontWeight:500,lineHeight:1.7,letterSpacing:"-0.2px",marginBottom:10}}>
    · <b style={{color:'#f59e0b'}}>A</b>: 2등 + 기+외만 + 1000억+ + 120일신고 + 등락 15-28%<br/>
    · <b style={{color:'#f59e0b'}}>B</b>: 2등 + 기관/기+외 + 500억+ + 점수3+ + 120일신고 + 등락 15-28%<br/>
    · <b style={{color:'#f59e0b'}}>C</b>: 1+2등 + 기관/기+외 + 500억+ + 점수3+ + 60일신고 + 등락 15-28%<br/>
    · 합집합 (A∪B∪C) — 5년 모두 양수 / 22년 +0.29% / 손익비 2.00
  </div>):(<div style={{fontSize:13,color:_T.body,fontWeight:500,lineHeight:1.7,letterSpacing:"-0.2px",marginBottom:10}}>
  ...
  <div style={{display:"flex",gap:8,alignItems:"baseline",marginBottom:6}}>
    <span style={{fontSize:13,fontWeight:700,color:tab.col,letterSpacing:"-0.2px"}}>💰 청산 룰</span>
  </div>
  {activeTab==='leader'?(<div style={{fontSize:13,color:_T.body,fontWeight:500,lineHeight:1.7,letterSpacing:"-0.2px"}}>
  ...
  </div>):activeTab==='best01'?(<div style={{fontSize:13,color:_T.body,fontWeight:500,lineHeight:1.7,letterSpacing:"-0.2px"}}>
  ...
  </div>):(<div style={{fontSize:13,color:_T.body,fontWeight:500,lineHeight:1.7,letterSpacing:"-0.2px"}}>
...
{activeTab==='leader'?(()=>{
  ...
  return(<div style={{marginBottom:18}}>
    <div style={{padding:"12px 14px",background:_T.card,border:"1px solid "+_T.line,borderRadius:11,marginBottom:8}}>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
        <span style={{fontSize:13,fontWeight:700,color:_T.text,letterSpacing:"-0.2px"}}>💰 자본</span>
        ...
        <span style={{fontSize:13,fontWeight:600,color:_T.sub}}>원</span>
      </div>
      <div style={{display:"flex",gap:6,fontSize:12,fontWeight:600,color:_T.hint,letterSpacing:"-0.2px"}}>
      ...
      return(<div key={s.code+i} style={{padding:"14px 16px",borderRadius:12,border:"1px solid "+_T.line,marginBottom:8,background:_T.card}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10,marginBottom:10}}>
          <div style={{flex:1,minWidth:0,display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
            ...
            <span title={exitR.sub} style={{fontSize:12,fontWeight:800,color:"#fff",background:exitR.col,padding:"3px 8px",borderRadius:4,letterSpacing:"-0.2px"}}>{exitR.l}</span>
            <span style={{fontSize:18,fontWeight:800,color:_T.text,letterSpacing:"-0.4px"}}>{s.name}</span>
            <span style={{fontSize:13,fontWeight:600,color:_T.hint,fontFamily:"ui-monospace,monospace"}}>{s.code}</span>
            <span style={{fontSize:12,fontWeight:600,color:_T.sub,padding:"2px 7px",borderRadius:4,background:_T.bg,border:"1px solid "+_T.line}}>{s._mktLabel}</span>
          </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,padding:"10px 12px",background:_T.bg,borderRadius:9,border:"1px solid "+_T.line}}>
          <div>
            <div style={{fontSize:12,color:_T.hint,fontWeight:600,marginBottom:3,letterSpacing:"-0.2px"}}>매수 비중</div>
            <div style={{fontSize:14,fontWeight:800,color:rankCol,letterSpacing:"-0.3px"}}>{(w*100).toFixed(1)}% <span style={{fontSize:13,color:_T.sub,fontWeight:600,marginLeft:3}}>{_man(amt)}원</span></div>
          </div>
          <div>
            <div style={{fontSize:12,color:_T.hint,fontWeight:600,marginBottom:3,letterSpacing:"-0.2px"}}>거래대금 · 수급</div>
            <div style={{fontSize:13,fontWeight:700,color:_T.body,letterSpacing:"-0.2px"}}>{s.amount}억 <span style={{fontSize:12,color:_T.sub,marginLeft:4}}>{s.investor||"—"}</span></div>
          </div>
        </div>
      </div>);
    })}
  ...
  return(<div style={{marginBottom:18}}>
    <div style={{padding:"10px 12px",background:'rgba(245,158,11,0.10)',border:'1px solid #f59e0b',borderRadius:10,marginBottom:8,fontSize:13,color:_T.body,fontWeight:600,letterSpacing:"-0.2px"}}>
      ...
      return(<div key={s.code+i} style={{padding:"14px 16px",borderRadius:12,border:"1px solid "+_T.line,marginBottom:8,background:_T.card}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10,marginBottom:10}}>
          <div style={{flex:1,minWidth:0,display:"flex",alignItems:"center",gap:7,flexWrap:"wrap"}}>
            ...
            <span style={{fontSize:12,fontWeight:800,color:"#fff",background:s._tierCol,padding:"3px 8px",borderRadius:4,letterSpacing:"-0.2px"}}>{s._tierLabel}</span>
            <span title={exitR.sub} style={{fontSize:12,fontWeight:800,color:"#fff",background:exitR.col,padding:"3px 8px",borderRadius:4,letterSpacing:"-0.2px"}}>{exitR.l}</span>
            <span style={{fontSize:18,fontWeight:800,color:_T.text,letterSpacing:"-0.4px"}}>{s.name}</span>
            <span style={{fontSize:13,fontWeight:600,color:_T.hint,fontFamily:"ui-monospace,monospace"}}>{s.code}</span>
            <span style={{fontSize:12,fontWeight:600,color:_T.sub,padding:"2px 6px",borderRadius:4,background:_T.bg,border:"1px solid "+_T.line}}>{s._mktLabel}</span>
          </div>
          <div style={{fontSize:14,fontWeight:800,color:_T.up,letterSpacing:"-0.3px"}}>+{(+s.change).toFixed(2)}%</div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,padding:"10px 12px",background:_T.bg,borderRadius:9,border:"1px solid "+_T.line,fontSize:13,fontWeight:600}}>
          <div><div style={{fontSize:13,color:_T.hint,fontWeight:600,marginBottom:2}}>거래대금</div><div style={{fontSize:13,fontWeight:800,color:_T.text}}>{s.amount}억</div></div>
          <div><div style={{fontSize:13,color:_T.hint,fontWeight:600,marginBottom:2}}>수급 · 점수</div><div style={{fontSize:12,fontWeight:700,color:_T.body}}>{s.supply||'—'} · {s.score||0}점</div></div>
          <div><div style={{fontSize:13,color:_T.hint,fontWeight:600,marginBottom:2}}>신고가</div><div style={{fontSize:13,fontWeight:700,color:_T.body}}>{s.h120===1?'120일 ✅':s.h60===1?'60일':'—'}</div></div>
        </div>
        <div style={{marginTop:8,fontSize:12,color:_T.sub,fontWeight:500,padding:'5px 10px',background:_T.linelt,borderRadius:6,letterSpacing:"-0.2px"}}>
...
{data.cetc&&data.cetc.length>0&&(<details style={{marginBottom:10}}>
  ...
  <div style={{padding:"6px 0"}}>{data.cetc.map((s,i)=>(
    <div key={s.code+i} onClick={()=>onSignalClick&&onSignalClick(s.code)} style={{padding:"10px 14px",fontSize:13,borderBottom:"1px solid "+_T.line,cursor:"pointer",background:_T.card,marginBottom:2,borderRadius:8}}>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4,flexWrap:"wrap"}}>
        <span style={{fontWeight:700,color:_T.body,fontSize:12}}>{s.name}</span>
        <span style={{fontFamily:"ui-monospace,monospace",fontSize:12,background:_T.bg,padding:"2px 6px",borderRadius:4,color:_T.hint,border:"1px solid "+_T.line}}>{s.code}</span>
        <span style={{color:_T.up,fontWeight:700,fontSize:12}}>+{s.change}%</span>
        <span style={{color:_T.mute,fontSize:12}}>{s.investor||"—"} · {s.amount}억 · 점수 {s.score}</span>
      </div>
      <div style={{fontSize:12,color:_T.up,fontWeight:600,letterSpacing:"-0.2px",opacity:0.85}}>⚠️ {s.failReason||"기타"}</div>
    </div>
  ))}</div>
</details>)}
</>)}
  ...
  return (
    <div>
      ...
      <div style={{display:"flex", alignItems:"center", gap:8, marginBottom:10, padding:"8px 10px", background:"#fffbeb", border:"1px solid #fde68a", borderRadius:8, fontSize:12, flexWrap:"wrap"}}>
        ...
        <span style={{color:"#92400e", fontSize:13}}>{backDate ? "🔍 백테스트 모드 - 이 날짜 기준 분석 + 익일 검증" : "(비워두면 오늘 기준 분석)"}</span>
        {backDate && <button onClick={()=>{setBackDate("");setVerifyResult(null);}} style={{padding:"3px 8px", background:"#fff", border:"1px solid #cbd5e1", borderRadius:6, fontSize:13, cursor:"pointer"}}>✖</button>}
      </div>
      ...
      <div onClick={() => fileRef.current && fileRef.current.click()}
        ...
        {imgs.length === 0 ? (
          <div>
            ...
            <div style={{fontSize:13, color:"#64748b", marginTop:6, fontWeight:600}}>AI분석 + 침착해 + 주도주 3중 동시 실행</div>
          </div>
        ) : (
          <div style={{display:"flex", gap:6, flexWrap:"wrap", justifyContent:"center"}}>
            {imgs.map((img, i) => (
              <div key={i} style={{position:"relative"}}>
                ...
                <button onClick={e => {e.stopPropagation(); setImgs(prev => prev.filter((_,j) => j !== i))}} style={{position:"absolute", top:-5, right:-5, width:18, height:18, borderRadius:9, border:"none", background:"#dc2626", color:"#fff", fontSize:13, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center"}}>×</button>
              </div>
            ))}
        ...
        <div>
          ...
          <div style={{display:"flex", gap:0, marginBottom:14, borderBottom:"2px solid #e2e8f0", overflowX:"auto"}}>
            <button onClick={() => setActiveTab("ai")} style={{flex:"1 0 auto", minWidth:80, padding:"12px 8px", border:"none", background:"transparent", borderBottom: activeTab==="ai" ? "3px solid #1e293b" : "3px solid transparent", marginBottom:"-2px", fontSize:12, fontWeight: activeTab==="ai" ? 800 : 600, color: activeTab==="ai" ? "#1e293b" : "#94a3b8", cursor:"pointer", fontFamily:"inherit", whiteSpace:"nowrap"}}>
              🧠 네오분석 {aiResult && <span style={{fontSize:13, color: gC(aiResult.grade), fontWeight:900, marginLeft:4}}>{aiResult.grade}</span>}
            </button>
            <button onClick={() => setActiveTab("chim")} style={{flex:"1 0 auto", minWidth:80, padding:"12px 8px", border:"none", background:"transparent", borderBottom: activeTab==="chim" ? "3px solid #7c3aed" : "3px solid transparent", marginBottom:"-2px", fontSize:12, fontWeight: activeTab==="chim" ? 800 : 600, color: activeTab==="chim" ? "#7c3aed" : "#94a3b8", cursor:"pointer", fontFamily:"inherit", whiteSpace:"nowrap"}}>
              🎯 침착해 {chimResult && <span style={{fontSize:13, fontWeight:900, marginLeft:4}}>{chimResult.grade}</span>}
            </button>
            <button onClick={() => setActiveTab("jd")} style={{flex:"1 0 auto", minWidth:80, padding:"12px 8px", border:"none", background:"transparent", borderBottom: activeTab==="jd" ? "3px solid #ca8a04" : "3px solid transparent", marginBottom:"-2px", fontSize:12, fontWeight: activeTab==="jd" ? 800 : 600, color: activeTab==="jd" ? "#ca8a04" : "#94a3b8", cursor:"pointer", fontFamily:"inherit", whiteSpace:"nowrap"}}>
              📈 주도주 {jdResult && <span style={{fontSize:13, fontWeight:900, marginLeft:4}}>{jdResult.grade}</span>}
            </button>
            <button onClick={() => setActiveTab("hs")} style={{flex:"1 0 auto", minWidth:80, padding:"12px 8px", border:"none", background:"transparent", borderBottom: activeTab==="hs" ? "3px solid #0d9488" : "3px solid transparent", marginBottom:"-2px", fontSize:12, fontWeight: activeTab==="hs" ? 800 : 600, color: activeTab==="hs" ? "#0d9488" : "#94a3b8", cursor:"pointer", fontFamily:"inherit", whiteSpace:"nowrap"}}>
              ⚡ 하승훈 {hsResult && <span style={{fontSize:13, fontWeight:900, marginLeft:4}}>{hsResult.grade}</span>}
            </button>
          </div>
              ...
              {aiResult && (
                <div style={{borderRadius:14, border:"2px solid " + gC(aiResult.grade), overflow:"hidden", marginBottom:14}}>
                  <div style={{background: gC(aiResult.grade), padding:"14px 18px", display:"flex", justifyContent:"space-between", alignItems:"center"}}>
                    <div>
                      <div style={{fontSize:13, color:"rgba(255,255,255,0.85)", fontWeight:700, marginBottom:2}}>🧠 네오분석 v1</div>
                      <div style={{fontSize:18, fontWeight:900, color:"#fff"}}>{aiResult.name || "분석 결과"}</div>
                      <div style={{fontSize:13, color:"rgba(255,255,255,0.85)", marginTop:2}}>{aiResult.breakType} · {aiResult.investor} · {aiResult.ema50}</div>
                    </div>
                    ...
                    <div style={{display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:6, marginBottom:10}}>
                      {[{l:"TP1", v:aiResult.tp1+"%"}, {l:"TP2", v:aiResult.tp2+"%"}, {l:"SL", v:aiResult.sl+"%"}].map((x, i) => (
                        <div key={i} style={{textAlign:"center", padding:8, background:"#f8fafc", borderRadius:8}}>
                          <div style={{fontSize:12, color:"#94a3b8"}}>{x.l}</div>
            ...
            <div style={{marginTop:12, padding:14, background:"#fefefe", border:"2px solid #c4b5fd", borderRadius:10, width:"100%", flexBasis:"100%", boxSizing:"border-box"}}>
              ...
              {aiResult.detailedAnalysis && (
                <div style={{marginBottom:12}}>
                  <div style={{fontSize:13, fontWeight:600, color:"#64748b", marginBottom:4}}>📋 종합 분석</div>
                ...
                <div style={{display:"grid", gridTemplateColumns:"repeat(2, 1fr)", gap:8, marginBottom:12}}>
                  ...
                  {aiResult.verdict && (
                    <div style={{padding:8, background:"#fef3c7", borderRadius:6, textAlign:"center"}}>
                      <div style={{fontSize:12, color:"#92400e"}}>판정</div>
              ...
              {Array.isArray(aiResult.keyReasons) && aiResult.keyReasons.length > 0 && (
                <div style={{marginBottom:12}}>
                  <div style={{fontSize:13, fontWeight:600, color:"#059669", marginBottom:4}}>✅ 핵심 이유</div>
              ...
              {Array.isArray(aiResult.risks) && aiResult.risks.length > 0 && (
                <div style={{marginBottom:12}}>
                  <div style={{fontSize:13, fontWeight:600, color:"#dc2626", marginBottom:4}}>⚠️ 리스크</div>
                  <ul style={{margin:0, paddingLeft:18, fontSize:12, color:"#334155", lineHeight:1.7}}>
              ...
              {aiResult.technicalIndicators && typeof aiResult.technicalIndicators === "object" && (
                <div style={{marginBottom:12}}>
                  <div style={{fontSize:13, fontWeight:600, color:"#0284c7", marginBottom:4}}>📊 기술적 지표</div>
              ...
              {aiResult.supplyZone && typeof aiResult.supplyZone === "object" && (
                <div style={{marginBottom:12}}>
                  <div style={{fontSize:13, fontWeight:600, color:"#9333ea", marginBottom:4}}>🧱 매물대 분석</div>
              ...
              {aiResult.strategy && typeof aiResult.strategy === "object" && (
                <div style={{marginBottom:6}}>
                  <div style={{fontSize:13, fontWeight:600, color:"#ea580c", marginBottom:4}}>🎯 매매 전략</div>
            ...
            <div style={{marginTop:14, padding:16, background:"#dcfce7", border:"1px solid #16a34a", borderRadius:10}}>
              ...
              {Array.isArray(verifyResult.days) && verifyResult.days.length > 0 && (
                <div style={{background:"#fff", borderRadius:8, padding:10}}>
                  <div style={{fontSize:13, fontWeight:700, color:"#14532d", marginBottom:6}}>?? ?? ??</div>
                  <div style={{display:"grid", gridTemplateColumns:"50px 1fr 1fr 1fr 1fr 80px", gap:4, fontSize:12, fontWeight:700, color:"#64748b", paddingBottom:4, borderBottom:"1px solid #e2e8f0"}}>
                  ...
                  {verifyResult.days.map((d, i) => {
                    ...
                    return (
                      <div key={i} style={{display:"grid", gridTemplateColumns:"50px 1fr 1fr 1fr 1fr 80px", gap:4, fontSize:12, padding:"4px 0", borderBottom: i < (verifyResult.days.length - 1) ? "1px solid #f1f5f9" : "none"}}>
            ...
            <div style={{marginTop:14, padding:16, background:"#fef3c7", border:"1px solid #f59e0b", borderRadius:10}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                ...
                {finalResult && !finalLoading && (
                  <button onClick={generateFinal}
                    style={{padding:"6px 10px",background:"transparent",color:"#78350f",border:"1px solid #f59e0b",borderRadius:6,fontSize:13,cursor:"pointer"}}>
                    🔄 재생성
                  </button>
                )}
              </div>
              ...
              {finalResult && (
                <div>
                  <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:8,marginBottom:12}}>
                    <div style={{padding:10,background:"#fff",borderRadius:8,textAlign:"center"}}>
                      <div style={{fontSize:12,color:"#78350f"}}>최종등급</div>
                      <div style={{fontSize:24,fontWeight:700,color:"#78350f"}}>{finalResult.finalGrade}</div>
                    </div>
                    <div style={{padding:10,background:"#fff",borderRadius:8,textAlign:"center"}}>
                      <div style={{fontSize:12,color:"#78350f"}}>판정</div>
                    ...
                    <div style={{padding:10,background:"#fff",borderRadius:8,textAlign:"center",gridColumn:"span 2"}}>
                      <div style={{fontSize:12,color:"#78350f"}}>신뢰도</div>
                  ...
                  {finalResult.consensus && (<div style={{marginBottom:10}}><div style={{fontSize:13,fontWeight:600,color:"#78350f",marginBottom:4}}>🤝 4개 분석 종합</div><div style={{padding:10,background:"#fff",borderRadius:6,fontSize:12,lineHeight:1.7,color:"#3f2f0a"}}>{finalResult.consensus}</div></div>)}
                  {finalResult.marketContext && (<div style={{marginBottom:10}}><div style={{fontSize:13,fontWeight:600,color:"#78350f",marginBottom:4}}>🌐 시장 컨텍스트</div><div style={{padding:10,background:"#fff",borderRadius:6,fontSize:12,lineHeight:1.6,color:"#3f2f0a",fontStyle:"italic"}}>{finalResult.marketContext}</div></div>)}
                  ...
                  {finalResult.scenarios && (<div style={{marginBottom:10}}><div style={{fontSize:13,fontWeight:600,color:"#78350f",marginBottom:6}}>📊 익일 시나리오별 대응</div>{finalResult.scenarios.bullish && <div style={{padding:8,background:"#dcfce7",borderRadius:6,fontSize:13,marginBottom:4,color:"#14532d"}}><b>📈 강세:</b> {finalResult.scenarios.bullish}</div>}{finalResult.scenarios.neutral && <div style={{padding:8,background:"#f3f4f6",borderRadius:6,fontSize:13,marginBottom:4,color:"#1f2937"}}><b>➡️ 보합:</b> {finalResult.scenarios.neutral}</div>}{finalResult.scenarios.bearish && <div style={{padding:8,background:"#fee2e2",borderRadius:6,fontSize:13,marginBottom:4,color:"#7f1d1d"}}><b>📉 약세:</b> {finalResult.scenarios.bearish}</div>}</div>)}
                  {Array.isArray(finalResult.riskFactors) && finalResult.riskFactors.length > 0 && (<div style={{marginBottom:10}}><div style={{fontSize:13,fontWeight:600,color:"#dc2626",marginBottom:4}}>⚠️ 리스크 요인</div><ul style={{margin:0,paddingLeft:18,fontSize:12,color:"#3f2f0a",lineHeight:1.6}}>{finalResult.riskFactors.map((r,i)=><li key={i}>{r}</li>)}</ul></div>)}
                  {Array.isArray(finalResult.watchPoints) && finalResult.watchPoints.length > 0 && (<div><div style={{fontSize:13,fontWeight:600,color:"#0284c7",marginBottom:4}}>👀 모니터링 포인트</div><ul style={{margin:0,paddingLeft:18,fontSize:12,color:"#3f2f0a",lineHeight:1.6}}>{finalResult.watchPoints.map((r,i)=><li key={i}>{r}</li>)}</ul></div>)}
                </div>
              )}
            </div>
          )}
  ...
  return (
    <div>
      <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10}}>
        ...
        <button onClick={onClear} style={{padding:"5px 10px", borderRadius:6, border:"1px solid #fca5a5", background:"#fff", color:"#dc2626", fontSize:13, fontWeight:600, cursor:"pointer"}}>전체 삭제</button>
      </div>
        ...
        return (
          <div key={i} onClick={() => {setSel(i); setDetailTab(r.grade ? "ai" : (hasChim ? "chim" : (hasJd ? "jd" : (hasHs ? "hs" : "ai"))));}} style={{cursor:"pointer", display:"flex", alignItems:"center", gap:10, padding:"10px 14px", borderRadius:10, border:"1px solid #e2e8f0", marginBottom:6, background:"#fff"}}>
            ...
            <div style={{flex:1, minWidth:0}}>
              ...
              <div style={{fontSize:13, color:"#94a3b8", marginTop:1}}>{r.date} · {r.score}점 · {r.breakType || "-"} · {r.investor || "-"}</div>
            </div>
            <div style={{textAlign:"right", flexShrink:0, display:"flex", gap:4, flexDirection:"column", alignItems:"flex-end"}}>
              {hasChim && (
                <span style={{fontSize:12, padding:"2px 6px", borderRadius:4, background: cgC(r.chimchakhaeResult.grade) + "22", color: cgC(r.chimchakhaeResult.grade), fontWeight:800}}>
                  침 {r.chimchakhaeResult.grade}
                </span>
              )}
              {hasJd && (
                <span style={{fontSize:12, padding:"2px 6px", borderRadius:4, background: "#ca8a0422", color: "#ca8a04", fontWeight:800}}>
                  주 {r.judojuResult.grade}
                </span>
              )}
              {hasHs && (
                <span style={{fontSize:12, padding:"2px 6px", borderRadius:4, background: "#0d948822", color: "#0d9488", fontWeight:800}}>
                  하 {r.haseunghoonResult.grade}
                </span>
              )}
              <div style={{fontSize:13, color:"#dc2626", fontWeight:700}}>TP{r.tp1}/{r.tp2} · SL{r.sl}%</div>
            </div>
                  ...
                  <div style={{borderRadius:14, border:"2px solid " + gC(h[sel].grade), overflow:"hidden", marginBottom:14}}>
                    <div style={{background: gC(h[sel].grade), padding:"14px 18px", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:8}}>
                      <div style={{minWidth:0, flex:"1 1 auto"}}>
                        ...
                        <div style={{fontSize:13, color:"rgba(255,255,255,0.85)", marginTop:2}}>{h[sel].breakType} · {h[sel].investor} · {h[sel].ema50}</div>
                        <div style={{fontSize:12, color:"rgba(255,255,255,0.7)", marginTop:3}}>{h[sel].date}</div>
                      </div>
                        ...
                        <div style={{fontSize:12}}>
                          {h[sel].details.map((d, idx) => (
                            <div key={idx} style={{padding:"6px 0", borderBottom:"1px solid #f1f5f9"}}>
                              ...
                              {d.reason && <div style={{color:"#94a3b8", fontSize:13, lineHeight:1.4}}>{d.reason}</div>}
                            </div>
                          ))}
                        </div>
                      )}
              ...
              <div style={{display:"flex", borderBottom:"2px solid #e2e8f0", padding:"16px 16px 0", overflowX:"auto"}}>
                {h[sel].grade && (
                  <button onClick={() => setDetailTab("ai")} style={{flex:"1 0 auto", minWidth:80, padding:"10px 8px", border:"none", background:"transparent", borderBottom: detailTab==="ai" ? "3px solid #1e293b" : "3px solid transparent", marginBottom:"-2px", fontSize:12, fontWeight: detailTab==="ai" ? 800 : 600, color: detailTab==="ai" ? "#1e293b" : "#94a3b8", cursor:"pointer", fontFamily:"inherit", whiteSpace:"nowrap"}}>
                    🧠 네오분석 <span style={{fontSize:13, color: gC(h[sel].grade), fontWeight:900, marginLeft:4}}>{h[sel].grade}</span>
                  </button>
                )}
                {/* AI 상세 분석 in 히스토리 모달 */}
                ...
                <div style={{marginTop:14, padding:14, background:"#fef3c7", border:"1px solid #f59e0b", borderRadius:10}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:8,marginBottom:10}}>
                    <div style={{padding:8,background:"#fff",borderRadius:6,textAlign:"center"}}><div style={{fontSize:12,color:"#78350f"}}>최종등급</div><div style={{fontSize:20,fontWeight:700,color:"#78350f"}}>{h[sel].finalResult.finalGrade}</div></div>
                    <div style={{padding:8,background:"#fff",borderRadius:6,textAlign:"center"}}><div style={{fontSize:12,color:"#78350f"}}>판정</div><div style={{fontSize:13,fontWeight:700,color:"#78350f",marginTop:4}}>{h[sel].finalResult.verdict}</div></div>
                  </div>
                  ...
                  {h[sel].finalResult.consensus && <div style={{marginBottom:8}}><div style={{fontSize:13,fontWeight:600,color:"#78350f",marginBottom:3}}>🤝 종합 의견</div><div style={{padding:8,background:"#fff",borderRadius:6,fontSize:13,lineHeight:1.6}}>{h[sel].finalResult.consensus}</div></div>}
                  {h[sel].finalResult.marketContext && <div style={{marginBottom:8}}><div style={{fontSize:13,fontWeight:600,color:"#78350f",marginBottom:3}}>🌐 시장 컨텍스트</div><div style={{padding:8,background:"#fff",borderRadius:6,fontSize:13,fontStyle:"italic"}}>{h[sel].finalResult.marketContext}</div></div>}
                  {(h[sel].finalResult.buyTiming || h[sel].finalResult.buyStrategy) && (<div style={{marginBottom:8,padding:8,background:"#dcfce7",borderRadius:6}}><div style={{fontSize:13,fontWeight:600,color:"#15803d",marginBottom:4}}>🟢 매수 전략</div>{h[sel].finalResult.buyTiming && <div style={{fontSize:13,color:"#14532d"}}><b>타이밍:</b> {h[sel].finalResult.buyTiming}</div>}{h[sel].finalResult.buyStrategy && <div style={{fontSize:13,color:"#14532d"}}><b>전략:</b> {h[sel].finalResult.buyStrategy}</div>}{h[sel].finalResult.addBuy && <div style={{fontSize:13,color:"#14532d"}}><b>추가매수:</b> {h[sel].finalResult.addBuy}</div>}</div>)}
                  {h[sel].finalResult.exitPlan && (<div style={{marginBottom:8,padding:8,background:"#fef2f2",borderRadius:6}}><div style={{fontSize:13,fontWeight:600,color:"#991b1b",marginBottom:4}}>🔴 청산 계획</div>{h[sel].finalResult.exitPlan.tp1 && <div style={{fontSize:13,color:"#7f1d1d"}}><b>TP1:</b> {h[sel].finalResult.exitPlan.tp1}</div>}{h[sel].finalResult.exitPlan.tp2 && <div style={{fontSize:13,color:"#7f1d1d"}}><b>TP2:</b> {h[sel].finalResult.exitPlan.tp2}</div>}{h[sel].finalResult.exitPlan.sl && <div style={{fontSize:13,color:"#7f1d1d"}}><b>SL:</b> {h[sel].finalResult.exitPlan.sl}</div>}{h[sel].finalResult.exitPlan.timeStop && <div style={{fontSize:13,color:"#7f1d1d"}}><b>시간:</b> {h[sel].finalResult.exitPlan.timeStop}</div>}</div>)}
                  {h[sel].finalResult.scenarios && (<div style={{marginBottom:8}}><div style={{fontSize:13,fontWeight:600,color:"#78350f",marginBottom:4}}>📊 시나리오별 대응</div>{h[sel].finalResult.scenarios.bullish && <div style={{padding:6,background:"#dcfce7",borderRadius:4,fontSize:13,marginBottom:3}}><b>📈 강세:</b> {h[sel].finalResult.scenarios.bullish}</div>}{h[sel].finalResult.scenarios.neutral && <div style={{padding:6,background:"#f3f4f6",borderRadius:4,fontSize:13,marginBottom:3}}><b>➡️ 보합:</b> {h[sel].finalResult.scenarios.neutral}</div>}{h[sel].finalResult.scenarios.bearish && <div style={{padding:6,background:"#fee2e2",borderRadius:4,fontSize:13}}><b>📉 약세:</b> {h[sel].finalResult.scenarios.bearish}</div>}</div>)}
                  {Array.isArray(h[sel].finalResult.riskFactors) && (<div style={{marginBottom:6}}><div style={{fontSize:13,fontWeight:600,color:"#dc2626",marginBottom:3}}>⚠️ 리스크</div><ul style={{margin:0,paddingLeft:18,fontSize:13,lineHeight:1.5}}>{h[sel].finalResult.riskFactors.map((r,i)=><li key={i}>{r}</li>)}</ul></div>)}
                </div>
              )}
                ...
                {h[sel].chimchakhaeResult && h[sel].chimchakhaeResult.grade && (
                  <button onClick={() => setDetailTab("chim")} style={{flex:"1 0 auto", minWidth:80, padding:"10px 8px", border:"none", background:"transparent", borderBottom: detailTab==="chim" ? "3px solid #7c3aed" : "3px solid transparent", marginBottom:"-2px", fontSize:12, fontWeight: detailTab==="chim" ? 800 : 600, color: detailTab==="chim" ? "#7c3aed" : "#94a3b8", cursor:"pointer", fontFamily:"inherit", whiteSpace:"nowrap"}}>
                    🎯 침착해 <span style={{fontSize:13, color: cgC(h[sel].chimchakhaeResult.grade), fontWeight:900, marginLeft:4}}>{h[sel].chimchakhaeResult.grade}</span>
                  </button>
                )}
                {h[sel].judojuResult && h[sel].judojuResult.grade && (
                  <button onClick={() => setDetailTab("jd")} style={{flex:"1 0 auto", minWidth:80, padding:"10px 8px", border:"none", background:"transparent", borderBottom: detailTab==="jd" ? "3px solid #ca8a04" : "3px solid transparent", marginBottom:"-2px", fontSize:12, fontWeight: detailTab==="jd" ? 800 : 600, color: detailTab==="jd" ? "#ca8a04" : "#94a3b8", cursor:"pointer", fontFamily:"inherit", whiteSpace:"nowrap"}}>
                    📈 주도주 <span style={{fontSize:13, color: "#ca8a04", fontWeight:900, marginLeft:4}}>{h[sel].judojuResult.grade}</span>
                  </button>
                )}
                {h[sel].haseunghoonResult && h[sel].haseunghoonResult.grade && (
                  <button onClick={() => setDetailTab("hs")} style={{flex:"1 0 auto", minWidth:80, padding:"10px 8px", border:"none", background:"transparent", borderBottom: detailTab==="hs" ? "3px solid #0d9488" : "3px solid transparent", marginBottom:"-2px", fontSize:12, fontWeight: detailTab==="hs" ? 800 : 600, color: detailTab==="hs" ? "#0d9488" : "#94a3b8", cursor:"pointer", fontFamily:"inherit", whiteSpace:"nowrap"}}>
                    ⚡ 하승훈 <span style={{fontSize:13, color: "#0d9488", fontWeight:900, marginLeft:4}}>{h[sel].haseunghoonResult.grade}</span>
                  </button>
                )}
              </div>
            ) : null}
        ...
        <div style={{display:"flex",maxWidth:1080,width:"100%",overflowX:"auto"}}>
          {[{id:"ai",label:"네오 Ai분석",icon:"🤖"},{id:"report",label:"마켓리포트",icon:"🌏"},{id:"neobaedb",label:"네오스코어",icon:"🎯"},{id:"today",label:"네오 종배",icon:"🔥"},{id:"history",label:"히스토리",icon:"📋"}].map(t=>{
            ...
            return(<button key={t.id} onClick={()=>setPage(t.id)} style={{flex:"1 0 auto",minWidth:65,padding:"10px 0 8px",border:"none",background:"none",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:3,position:"relative"}}>
              ...
              <span style={{fontSize:13,fontWeight:_act?700:500,color:_act?_activeColor:_inactiveColor,letterSpacing:"-0.2px"}}>{t.label}</span>
              {t.badge>0&&<span style={{position:"absolute",top:6,right:"calc(50% - 18px)",background:"#f04452",color:"#fff",fontSize:13,fontWeight:700,padding:"0px 4px",borderRadius:8,minWidth:14,textAlign:"center"}}>{t.badge}</span>}
            </button>);
          })}
        </div>
      </div>
