...
function SignalDB(){
  ...
  const st=useMemo(()=>{const r={};["S","A","B"].forEach(g=>{const d=D.filter(x=>x.g===g),w=d.filter(x=>x.t>0),sl=d.filter(x=>x.r==="SL"),sl2=d.filter(x=>x.r==="SL2"),bo=d.filter(x=>x.r==="BOTH"),tp1=d.filter(x=>x.r==="TP1"||x.r==="BOTH"||x.r==="SL2"),to=d.filter(x=>x.r==="TO");const avg=d.length?(d.reduce((s,x)=>s+x.t,0)/d.length):0;r[g]={n:d.length,avg:avg.toFixed(2),wr:d.length?Math.round(w.length/d.length*100):0,cum:Math.round(d.reduce((s,x)=>s+x.t,0)),tp1c:tp1.length,tp1r:d.length?Math.round(tp1.length/d.length*100):0,boc:bo.length,bor:d.length?Math.round(bo.length/d.length*100):0,slc:sl.length,slr:d.length?Math.round(sl.length/d.length*100):0,sl2c:sl2.length,sl2r:d.length?Math.round(sl2.length/d.length*100):0,toc:to.length,tor:d.length?Math.round(to.length/d.length*100):0}});return r},[]);
  ...
  return(<div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(148px,1fr))",gap:8,marginBottom:18}}>
      {["S","A","B"].map(g=>{const i=GI[g],s=st[g];return(<div key={g} onClick={()=>{setTab(g);setPg(0);setOpen(null)}} style={{background:tab===g?i.c:i.bg,borderRadius:14,padding:"12px 14px",cursor:"pointer",border:tab===g?"none":"0.5px solid "+i.bd}}><div style={{display:"flex",justifyContent:"space-between"}}><span style={{fontSize:26,fontWeight:900,color:tab===g?"#fff":i.c}}>{g}</span><span style={{fontSize:10,padding:"2px 6px",borderRadius:5,background:tab===g?"rgba(255,255,255,0.2)":i.c+"15",color:tab===g?"#fff":i.c,fontWeight:600}}>{g==="B"?"TP12/50":"TP15/50"}</span></div><div style={{fontSize:22,fontWeight:900,color:tab===g?"#fff":i.c}}>{s.n}<span style={{fontSize:11,fontWeight:400}}>건</span></div><div style={{fontSize:11,color:tab===g?"rgba(255,255,255,0.8)":"#64748b",marginTop:2}}>+{s.avg}%/건 · 승률<strong>{s.wr}%</strong> · 누적+{s.cum}%</div><div style={{fontSize:10,color:tab===g?"rgba(255,255,255,0.65)":"#94a3b8",marginTop:1}}>1차{s.tp1c}({s.tp1r}%) · 양쪽{s.boc}({s.bor}%) · SL{s.slc}({s.slr}%)</div></div>)})}
    ...
    {st[tab]&&<div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:6,marginBottom:12}}>
      <div style={{background:"#f0fdf4",borderRadius:10,padding:"8px 10px",textAlign:"center",border:"1px solid #bbf7d0"}}><div style={{fontSize:10,color:"#059669"}}>익절 (1차+양쪽)</div><div style={{fontSize:18,fontWeight:900,color:"#059669"}}>{st[tab].tp1c}건</div><div style={{fontSize:10,color:"#64748b"}}>{st[tab].tp1r}% · 양쪽{st[tab].boc}건({st[tab].bor}%)</div></div>
      <div style={{background:"#fef2f2",borderRadius:10,padding:"8px 10px",textAlign:"center",border:"1px solid #fecaca"}}><div style={{fontSize:10,color:"#dc2626"}}>손절 (SL+SL2)</div><div style={{fontSize:18,fontWeight:900,color:"#dc2626"}}>{st[tab].slc+st[tab].sl2c}건</div><div style={{fontSize:10,color:"#64748b"}}>{Math.round((st[tab].slc+st[tab].sl2c)/st[tab].n*100)}% · SL2:{st[tab].sl2c}건</div></div>
      <div style={{background:"#f8fafc",borderRadius:10,padding:"8px 10px",textAlign:"center",border:"1px solid #e2e8f0"}}><div style={{fontSize:10,color:"#64748b"}}>기간만료 (TO)</div><div style={{fontSize:18,fontWeight:900,color:"#64748b"}}>{st[tab].toc}건</div><div style={{fontSize:10,color:"#94a3b8"}}>{st[tab].tor}%</div></div>
    </div>}
